...
def test_profitability_guardrails(trade_engine):
    ...
    # --- ROI Check ---
    roi = (report["equity"] - trade_engine.config["risk"]["equity"]) / trade_engine.config["risk"]["equity"]
    target = trade_engine.config["risk"]["target_daily_return"]

    assert roi >= target * 0.5, f"ROI {roi:.2%} below acceptable guardrail (target {target:.2%})"
    ...
    print(f"✅ Guardrails passed | ROI={roi:.2%}, Sharpe={sharpe:.2f}, Drawdown={dd:.2%}")
