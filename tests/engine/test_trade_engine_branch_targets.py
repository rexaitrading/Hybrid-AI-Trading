...
def test_helpers_and_outcomes(monkeypatch):
    ...
    m0 = eng.alert("")          ; assert isinstance(m0, dict)
    m1 = eng.alert("x"*256)     ; assert isinstance(m1, dict)

    # getters before/after outcomes
    eq0 = eng.get_equity()      ; assert isinstance(eq0, (int,float))
    eng.record_trade_outcome(+12.34)
    eng.record_trade_outcome(-7.89)
    eq1 = eng.get_equity()      ; assert isinstance(eq1, (int,float))

    # positions/history
    pos = eng.get_positions()   ; assert isinstance(pos, dict)
    hist = eng.get_history()    ; assert isinstance(hist, list)

    # reset twice to touch multiple reset paths
    rA = eng.reset_day()        ; assert isinstance(rA, dict)
    rB = eng.reset_day()        ; assert isinstance(rB, dict)
