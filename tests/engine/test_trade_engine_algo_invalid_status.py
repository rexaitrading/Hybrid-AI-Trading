...
def _install(monkeypatch, name, executor_cls):
    key = name.lower()
    cls = {"twap":"TWAPExecutor","vwap":"VWAPExecutor","iceberg":"IcebergExecutor"}[key]
    path = f"hybrid_ai_trading.algos.{key}"
    m = types.ModuleType(path); setattr(m, cls, executor_cls)
    if path in sys.modules: del sys.modules[path]
    sys.modules[path] = m; importlib.invalidate_caches()

class WeirdExec:  # returns status not in allowed set → invalid_status
