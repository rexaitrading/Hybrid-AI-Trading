...
def _build_engine(monkeypatch):
    ...
    class _Fallback:
        ...
        def sell_market(self, s, q):
            return self.om.sell_market(s, q)
...
def _maybe(callable_):
    try:
        ...
        # DonÃ¢â‚¬â„¢t fail sweep on optional paths; goal is coverage, not strict behavior here
        return None
...
def test_trade_engine_method_sweep(monkeypatch):
    ...
    if hasattr(eng, "buy_market"):
        _maybe(lambda: eng.buy_market("AAPL", 1))
    if hasattr(eng, "sell_market"):
        _maybe(lambda: eng.sell_market("AAPL", 1))
    if hasattr(eng, "buy_limit"):
        _maybe(lambda: eng.buy_limit("AAPL", 1.0, 123.45))
    if hasattr(eng, "sell_limit"):
        _maybe(lambda: eng.sell_limit("AAPL", 0.5, 321.00))
    if hasattr(eng, "positions"):
