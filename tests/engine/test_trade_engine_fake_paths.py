...
class _StubBroker:
    ...
    def place_order(self, symbol, side, qty, order_type="MARKET", limit_price=None, meta=None):
        ...
        self._orders.append({"orderId": oid, "symbol": symbol, "side": side.upper(),
                             "qty": float(qty), "type": order_type, "status": status})
        self._pos[symbol] = self._pos.get(symbol, 0.0) + (qty if side.upper()=="BUY" else -qty)
        return oid, {"status": status, "filled": float(qty if status=="Filled" else 0.0),
                     "avgPrice": 0.0, "meta": meta or {}}
    def open_orders(self): return list(self._orders)
    def positions(self): return [{"symbol": s, "position": q} for s,q in self._pos.items()]
...
def test_engine_exception_branch(monkeypatch):
    ...
    class _BoomBroker:
        ...
        def server_time(self): return "2025-10-11 00:00:00"
        def place_order(self, *a, **k): raise RuntimeError("synthetic failure")
...
def test_engine_bad_qty_behavior(monkeypatch):
    ...
    class _Stub:
        ...
        def place_order(self, symbol, side, qty, order_type="MARKET", limit_price=None, meta=None):
            ...
            oid = 1
            return oid, {"status":"Filled","filled": float(qty or 0), "avgPrice":0.0, "meta": meta or {}}
        def open_orders(self): return []
        def positions(self): return []
