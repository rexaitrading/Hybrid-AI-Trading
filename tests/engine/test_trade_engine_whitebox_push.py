...
class _FailOM:
    def buy_market(self, *a, **k):
        raise RuntimeError("om.buy_market fail")

    def sell_market(self, *a, **k):
        raise RuntimeError("om.sell_market fail")

    def buy_limit(self, *a, **k):
        raise RuntimeError("om.buy_limit fail")

    def sell_limit(self, *a, **k):
        raise RuntimeError("om.sell_limit fail")
...
def test_no_brokers_and_unknown_algo_paths(eng):
    # 1) wipe brokers/smart router if present Ã¢â€ â€™ trigger "no brokers"/fallback branches
    ...
    # 2) unknown/odd algos and symbol edges Ã¢â€ â€™ earlier else/guard branches
    cases = [
    ...
    for sym, sig, px, sz, algo in cases:
        try:
            ...
            # acceptable Ã¢â‚¬â€ we only need to touch the branches
            pass
...
def test_extreme_and_nonfinite_pnl_then_reset(eng):
    # Ã‚Â±extremes and non-finite Ã¢â€ â€™ guard branches in outcome/accumulators
