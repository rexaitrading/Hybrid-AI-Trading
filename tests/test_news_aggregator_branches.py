...
class _OKBZ:
    def get_news(self, symbols_csv, limit=10, date_from=None):
        return [{"id":"sid_dup","url":"https://dup.url","title":"META X","stocks":[{"name":"META","exchange":""}]}]

class _OKPG:
    def get_news(self, symbol, limit=5, date_from=None):
        return [{"id":"sid_dup","url":"https://diff.url","title":f"{symbol} Y","stocks":[{"name":symbol,"exchange":""}]}]
...
class _OKRSS:
    ...
    def get_news(self, symbols_csv, date_from=None):
        return [{"id":"rss1","url":"https://rss","title":"AAPL Z","stocks":[{"name":"AAPL","exchange":""}]}]
...
def cfg(tmp_path):
    ...
    cfg = {"news_providers":{
        ...
        "alpaca":   {"enabled": True},
        "rss":      {"enabled": True,"per_feed_max": 3,"google_template": None,"yahoo_template": None,"extra_feeds":[]},
    ...
    try:
        ...
        if backup is None:
            ...
            with open(CFG,"w",encoding="utf-8") as f: f.write(backup)
