import os, sys, types, importlib, inspect, io
import pytest
...
class PT:
    def __init__(self, s=-2.0, t=-2.0): self._s=s; self._t=t
...
def _mk():
    ...
    cfg = {
        "risk":{"max_drawdown":0.5,"sharpe_min":-1.0,"sortino_min":-1.0,"intraday_sector_exposure": 1.0},
        "alerts":{"slack_url":"http://x","telegram_bot":"b","telegram_chat":"c","email":True}
    }
    cm = {
        "portfolio":PF(), "risk_manager":RM(), "risk":RM(), "rm":RM(),
        "order_manager":OM(), "order_router":OM(), "router":OM(),
        "performance_tracker":PT(), "perf":PT(), "tracker":PT(),
        "config":cfg
    }
...
def test_audit_header_then_exception_154_169(monkeypatch, tmp_path):
    ...
    if hasattr(te,"_write_audit"): te._write_audit(["t","AAPL","BUY",1,1.0,"ok",100.0,""])
...
def test_router_error_and_algo_imports_263_288(monkeypatch):
    ...
    # router error
    if hasattr(te,"order_manager"): te.order_manager.route=lambda *a,**k: (_ for _ in ()).throw(RuntimeError("router"))
    if hasattr(te,"_route_direct"):
        r=te._route_direct("AAPL","BUY",1,1.0); assert r["status"]=="blocked"
