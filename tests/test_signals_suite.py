"""
Unit Tests: Signals Suite (Hybrid AI Quant Pro v25.0 – Final 100% Coverage Stable)
---------------------------------------------------------------------------------
...
breakout_polygon_module = importlib.import_module("hybrid_ai_trading.signals.breakout_polygon")
...
def test_breakout_v1_all(caplog, monkeypatch):
    ...
    assert breakout_v1_module.breakout_v1([]) == "HOLD"
    assert breakout_v1_module.breakout_v1(make_bars([1, 2, 3]), window=0) == "HOLD"
    assert breakout_v1_module.breakout_v1(make_bars([1, 2]), window=5) == "HOLD"
    assert breakout_v1_module.breakout_v1([{"c": "bad"}], window=1) == "HOLD"
    assert breakout_v1_module.breakout_v1(make_bars([float("nan"), 5, 10]), window=3) == "HOLD"

    assert breakout_v1_module.breakout_v1(make_bars([50, 50, 50]), window=3) == "SELL"
    assert breakout_v1_module.breakout_v1(make_bars([10, 12, 4]), window=3) == "SELL"
    assert breakout_v1_module.breakout_v1(make_bars([10, 12, 10]), window=3) == "HOLD"
    assert breakout_v1_module.breakout_v1(make_bars([1, 2, 10]), window=3) == "BUY"

    # Audit mode
    d, price, rh, rl = breakout_v1_module.breakout_v1(make_bars([1, 10, 5]), window=3, audit=True)
    assert d in ["BUY", "SELL", "HOLD"]

    # Wrapper fetch failure
    monkeypatch.setattr(
        breakout_v1_module, "get_ohlcv_latest", lambda *a, **k: (_ for _ in ()).throw(Exception("boom"))
    )
    assert breakout_v1_module.breakout_signal("AAPL") == "HOLD"
...
def test_polygon_all(mock_get, monkeypatch, caplog):
    ...
    monkeypatch.setattr(breakout_polygon_module, "get_polygon_bars", lambda *a, **k: [{"c": 100, "h": 101, "l": 99}])
    assert breakout_polygon_module.breakout_signal_polygon("AAPL") == "HOLD"

    monkeypatch.setattr(breakout_polygon_module, "get_polygon_bars", lambda *a, **k: [{"c": 100, "h": 101}] * 3)
    assert breakout_polygon_module.breakout_signal_polygon("AAPL") == "HOLD"

    monkeypatch.setattr(
        breakout_polygon_module, "get_polygon_bars", lambda *a, **k: [{"c": math.nan, "h": 101, "l": 99}] * 3
    )
    assert breakout_polygon_module.breakout_signal_polygon("AAPL") == "HOLD"
...
def test_macd_all(monkeypatch, caplog):
    ...
    class FakeEWM:
        def mean(self, *a, **k): return pd.Series([float("nan")] * 50)
...
def test_moving_average_all(monkeypatch, caplog):
    ...
    assert moving_average_signal([{"o": 1}] * 25, short_window=5, long_window=20) == "HOLD"

    bars = [{"c": i} for i in range(1, 50)]
    assert moving_average_signal(bars, short_window=3, long_window=10) in ["BUY", "SELL", "HOLD"]

    class FakeRolling:
        def mean(self): return pd.Series([float("nan")])

    monkeypatch.setattr(pd.Series, "rolling", lambda self, window: FakeRolling())
    assert moving_average_signal([{"c": i} for i in range(25)], short_window=5, long_window=20) == "HOLD"
