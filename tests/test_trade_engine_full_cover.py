...
def test_audit_header_and_exception(monkeypatch, tmp_path):
    ...
    if hasattr(te, "_write_audit"):
        te._write_audit(["t", "AAPL", "BUY", 1, 1.0, "ok", 100.0, ""])  # 154â€“167
    ...
    if hasattr(te, "_write_audit"):
        te._write_audit(["t", "AAPL", "BUY", 1, 1.0, "ok", 100.0, ""])  # 168â€“169
...
def test_base_fraction_and_helpers_reflective():
    ...
    for name in dir(te):
        ...
        want = (
            any(k in name.lower() for k in ("fraction", "kelly", "gate", "base"))
            or _safe_param_len(fn) <= 3
        )
        if not want:
            continue
