import sys, types, importlib, inspect, os, pytest
...
def test_targeted_clusters(monkeypatch, tmp_path):
    ...
    defaults = {
        ...
        "notional": 100.0, "side": "BUY",
        "bar_ts": 1_000_000, "bar_ts_ms": 1_000_000,
    ...
    for name in dir(te):
        if "alert" in name.lower(): _invoke(getattr(te,name), {"message":"ok"})
    ...
    for name in dir(te):
        if "audit" in name.lower():
            try: _invoke(getattr(te,name), defaults)
            except Exception: pass
    ...
    # -------- equity depleted (236)
    te0 = make_engine(); te0.portfolio.equity = 0.0
    ...
    te3 = make_engine(risk_override={"intraday_sector_exposure":0.001},
    ...
    if f:
        f("AAPL","BUY",1,1.0, algo="twap"); f("AAPL","BUY",1,1.0, algo="vwap")
    monkeypatch.setattr(importlib,"import_module",
    ...
    te6 = make_engine()
    te6.config.setdefault("filters",{}); te6.config["filters"].update({"sentiment":True,"gatescore":True})
    ...
    te6.performance_tracker = type("PT",(object,),{
        "sharpe_ratio": lambda s:-2.0, "sortino_ratio": lambda s:-2.0, "record_trade": lambda s,p: None})()
    ...
    if hasattr(te6,"get_positions"): te6.get_positions()
    if hasattr(te6,"get_history"):   te6.get_history()
    te6.performance_tracker.record_trade = lambda pnl: (_ for _ in ()).throw(RuntimeError("x"))
    if hasattr(te6,"record_trade_outcome"): te6.record_trade_outcome(1.23)
