...
def test_finish_process_signal_all_branches():
    ...
    # A) regime disabled â†’ early return (~301)
    _prep(te)
    ...
    # B) sortino breach â†’ blocked (~325)
    _prep(te)
    ...
    # C) drawdown gate present & Kelly exception â†’ size fallback (241â€“257)
    _prep(te)
    ...
    # D) tail normalization: result.status/reason "ok" -> "filled"/"normalized_ok" (334â€“339)
    _prep(te)
    te.config["regime"]["enabled"] = True
    if hasattr(te, "risk_manager"):
