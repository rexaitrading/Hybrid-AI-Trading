"""
Unit Tests: Clients (Hybrid AI Quant Pro v9.0 - Absolute 100% Coverage)
-----------------------------------------------------------------------
Covers:
- AlpacaClient:
  * Init with missing keys (warn branch)
  * _mask_key (None, short, long)
  * _headers with/without creds
  * _request success/failure
  * ping() → True, AlpacaAPIError, generic Exception
- PolygonClient:
  * Init missing key (raises RuntimeError)
  * prev_close success + failure
  * ping() → True, PolygonAPIError, generic Exception
- BenzingaClient:
  * Init missing key raises
  * get_news success + failure
- CoinAPI Client:
  * _get_headers: valid key, missing key, config error, load_config exception
  * _iso helper
  * _retry_get: success, retryable fail, non-retryable fail
  * get_fx_rate: success, bad JSON
  * get_ohlcv_latest: success, error dict, candidate failures
  * ping() True + False
"""
...
def test_alpaca_mask_key_variants():
    ...
    assert client._mask_key(None) == "None"
    assert client._mask_key("short") == "short"
...
def test_benzinga_success_and_fail(mock_get):
    ...
    assert isinstance(client.get_news("AAPL"), list)
...
# CoinAPI Client
# ==========================================================
def test_coinapi_headers_variants(monkeypatch):
...
@patch("hybrid_ai_trading.data.clients.coinapi_client.get_fx_rate")
