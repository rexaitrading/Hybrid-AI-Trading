...
def test_cooldown_expiry_clears_state(tmp_path):
    rm = RiskManager(RiskConfig(state_path=str(tmp_path/"b.json"), cooldown_bars=1, max_consecutive_losers=1))
    ...
    assert ok2 and rm._state.get("halted_until_bar_ts") is None and rm._state.get("halted_reason") is None
