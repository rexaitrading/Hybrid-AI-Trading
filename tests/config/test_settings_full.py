...
def test_validate_bad_neutral_zone(nz):
    with pytest.raises(ValueError):
        validate_config(
            {"mode": "paper", "sentiment": {"model": "vader", "neutral_zone": nz}}
        )
...
def test_validate_bad_threshold(thr):
    with pytest.raises(ValueError):
        validate_config(
            {"mode": "paper", "sentiment": {"model": "vader", "threshold": thr}}
        )
...
def test_validate_bad_starting_equity(se):
    with pytest.raises(ValueError):
        validate_config(
            {"mode": "paper", "sentiment": {"model": "vader"}, "starting_equity": se}
        )
    ...
    out = validate_config(
        {
            ...
            "costs": {"commission_pct": 0.001, "slippage_pct": 0},
        }
    )
