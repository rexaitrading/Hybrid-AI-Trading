"""
Unit Tests: settings.py (Hybrid AI Quant Pro â€“ 100% Coverage)
-------------------------------------------------------------
Covers:
- Missing config file
- Config returns non-dict
- YAML parsing error
- File open exception (new â†’ covers lines 45â€“47)
...
def test_file_open_exception(monkeypatch, caplog):
    """Covers the bare except branch (lines 45â€“47)."""
    ...
    with caplog.at_level("ERROR"):
        cfg = settings.load_config()
    assert cfg == {}
    # âœ… Match the actual log message from settings.py
