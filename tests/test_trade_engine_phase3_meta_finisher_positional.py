...
def _pos_args_for(fn):
    ...
    for i, (name, param) in enumerate(sig.parameters.items()):
        ...
        if name in sup:
            ...
            # generic, but harmless fallback â€” strings for likely names, else simple numerics
...
# -------- (241â†’251) drawdown block executes WITHOUT breach, then continues to sizing -----------
...
# -------- (247â€“248) drawdown try/except path: malformed history triggers except ----------
def test_ps_drawdown_except_path_malformed_history_positional():
    te = make_engine()
    _neutralize(te)
    te.config["risk"]["max_drawdown"] = 0.99  # gate present
    # history is a string => indexing [0][1] raises -> except path (247â€“248)
...
# -------- (334â€“339) tail normalization (status/ reason "ok" -> "filled"/"normalized_ok") ----------
