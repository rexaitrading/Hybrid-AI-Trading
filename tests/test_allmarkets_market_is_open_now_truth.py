...
def _json_from_ps(args: list[str]) -> dict:
    return json.loads(_run_ps(args).strip())
...
def test_allmarkets_market_is_open_now_truth():
    ...
    for mkt in ["US", "JP", "HK", "SG"]:
        ...
        assert bool(st.get("market_is_open_now")) == bool(mc.get("is_open_now")), (mkt, st.get("market_is_open_now"), mc.get("is_open_now"))
