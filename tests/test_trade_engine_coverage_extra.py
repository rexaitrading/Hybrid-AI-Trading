import os, sys, types, importlib, inspect
import pytest
...
class DummyPT:
    ...
    def sharpe_ratio(self): return self._sharpe
    def sortino_ratio(self): return self._sortino
...
def _find_signal_method(te):
    ...
    names = ["process_signal","_on_signal","on_signal","handle_signal","submit_signal","submit","trade","run_signal","evaluate_signal"]
    for n in names:
        f = getattr(te, n, None)
    ...
    if f is not None:
        try:
            ...
            for p in list(sig.parameters.values()):
                if p.name == "symbol": order.append(symbol)
                elif p.name == "size": order.append(size)
                elif p.name == "price": order.append(price)
                elif p.name == "signal": order.append(signal)
            return f(*order)
...
def test_reject_invalid_signal_and_price_paths():
    ...
    assert call_signal(te, symbol="AAPL", size=1.0, price=None, signal="BUY")["status"] == "rejected"
...
def test_alert_channels_success_and_fail(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "requests", types.SimpleNamespace(post=fake.post, get=fake.get))
    class DummySMTP:
        def __enter__(self): raise RuntimeError("smtp down")
        def __exit__(self, *a): return False
    monkeypatch.setitem(sys.modules, "smtplib", types.SimpleNamespace(SMTP=lambda *a, **k: DummySMTP()))
...
def test_algo_dynamic_import_paths(monkeypatch):
    ...
    class VWAPExecutor(TWAPExecutor): pass
...
def test_router_error_branch():
    te = make_engine()
    def boom(*a, **k): raise RuntimeError("router-bad")
...
def test_positions_history_and_record_trade_paths(caplog):
    ...
    def bad_record(pnl): raise RuntimeError("bad rec")
