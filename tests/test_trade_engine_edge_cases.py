...
def test_algo_import_failures(engine, monkeypatch):
    ...
    monkeypatch.setattr(importlib, "import_module", lambda *_: (_ for _ in ()).throw(ImportError("fail")))
...
def test_sentiment_and_gatescore_exceptions(engine, monkeypatch):
    ...
    monkeypatch.setattr(engine.sentiment_filter, "allow_trade", lambda *_: (_ for _ in ()).throw(Exception("sentiment fail")))
    ...
    monkeypatch.setattr(engine.gatescore, "allow_trade", lambda *_: (_ for _ in ()).throw(Exception("gatescore fail")))
...
def test_record_trade_outcome_exception(engine, monkeypatch, caplog):
    ...
    caplog.set_level("ERROR")
    monkeypatch.setattr(engine.performance_tracker, "record_trade", lambda *_: (_ for _ in ()).throw(Exception("boom")))
...
def test_performance_tracker_exception(engine, monkeypatch):
    ...
    monkeypatch.setattr(engine.performance_tracker, "sharpe_ratio", lambda *_: (_ for _ in ()).throw(Exception("boom")))
    res = engine.process_signal("AAPL", "BUY", 100)
    assert "status" in res
...
def test_performance_tracker_sortino_exception(engine, monkeypatch):
    ...
    monkeypatch.setattr(engine.performance_tracker, "sortino_ratio", lambda *_: (_ for _ in ()).throw(Exception("boom")))
    res = engine.process_signal("AAPL", "BUY", 100)
    assert "status" in res
...
def test_record_trade_outcome_logs_error(engine, monkeypatch, caplog):
    ...
    caplog.set_level("ERROR")
    monkeypatch.setattr(engine.performance_tracker, "record_trade", lambda *_: (_ for _ in ()).throw(Exception("fail")))
