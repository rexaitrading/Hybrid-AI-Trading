...
def test_alerts_success_and_exceptions(monkeypatch):
    ...
    if hasattr(te,"_fire_alert"): te._fire_alert("A")
    ...
    if hasattr(te,"_fire_alert"): te._fire_alert("B")

# ---------- 3) AUDIT: header + exception (154–169) ----------
...
# ---------- 7) FILTER VETO (306/312) + RATIO VETO (317–326) + audit capture (352) ----------
def test_filters_ratios_and_audit_capture(monkeypatch,tmp_path):
    ...
    if hasattr(te,"_write_audit"):
        ...
        if hasattr(te,"_fire_alert"): te._fire_alert("audit capture")

# ---------- 8) POSITIONS/HISTORY (376/379) + trade outcome log (384–387) ----------
def test_positions_history_outcome(caplog):
    te=_mk()
    if hasattr(te,"get_positions"): te.get_positions()
    if hasattr(te,"get_history"):   te.get_history()
...
# ---------- 9) RESET DAY branches: portfolio, risk, generic (175–198) ----------
...
# ---------- 10) FUZZER: best-effort touch of miscellaneous helpers (incl. 201–212, 368, 373) ----------
...
def kwarg_only(sig):
    ...
    return all(k in (inspect.Parameter.KEYWORD_ONLY, inspect.Parameter.POSITIONAL_OR_KEYWORD, inspect.Parameter.VAR_KEYWORD, inspect.Parameter.VAR_POSITIONAL) for k in kinds)