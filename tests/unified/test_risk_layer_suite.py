...
from hybrid_ai_trading.execution.portfolio_tracker import PortfolioTracker   # ✅ FIX ADDED
...
class DummyOrderManager:
    def place_order(self, *args, **kwargs): return {"status": "filled"}
...
@pytest.mark.parametrize("win_rate,wl,regime",
...
def test_vwap_executor_flows(dummy_order_manager):
    ...
    assert vwap.execute("AAPL", "BUY", 10, 200)["status"] in {"filled", "rejected", "error"}
    assert vwap.execute("TSLA", "SELL", 10, 100)["status"] in {"filled", "rejected", "error"}
    assert vwap.execute("MSFT", "BUY", 10, 10)["status"] in {"filled", "rejected", "error"}
...
def test_vwap_executor_unexpected_and_exception(monkeypatch, dummy_order_manager, caplog):
    ...
    monkeypatch.setattr("hybrid_ai_trading.algos.vwap_executor.vwap_signal",
...
def test_black_swan_guard_trigger_clear_and_filter(caplog):
    ...
    assert guard.active()
    guard.clear_event("news"); assert not guard.active()
    guard.trigger_event("macro", "inflation"); guard.clear_all()
    assert not guard.active()
...
def test_gatescore_partial_and_adaptive(monkeypatch):
    gs = GateScore(models=["sentiment", "price"], weights={"sentiment": 0.5, "price": 0.5},
                   threshold=0.7, adaptive=True)
...
def test_sortino_guard_blocks_and_exception(caplog):
    class RM(RiskManager):
        def sortino_ratio(self): return 0.2
