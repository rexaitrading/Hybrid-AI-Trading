"""
Unit Tests: Moving Average Signal (Hybrid AI Quant Pro v23.7 Ã¢â‚¬â€œ Hedge-Fund Grade, 100% Coverage)
-----------------------------------------------------------------------------------------------
...
def test_forced_buy_branch(monkeypatch):
    """Force short MA > long MA Ã¢â€ â€™ BUY."""
    bars = make_bars(30)
...
def test_forced_sell_branch(monkeypatch):
    """Force short MA < long MA Ã¢â€ â€™ SELL."""
    bars = make_bars(30)
...
def test_hold_equal_forced(monkeypatch):
    """Force short MA == long MA Ã¢â€ â€™ HOLD."""
    bars = make_bars(30)
...
def test_nan_sma(monkeypatch):
    """Force rolling().mean() to produce NaN Ã¢â€ â€™ HOLD."""
    bars = make_bars(30)
...
def test_rolling_failure(monkeypatch):
    """Simulate exception in rolling mean Ã¢â€ â€™ triggers 'failed rolling'."""
    bars = make_bars(30)
