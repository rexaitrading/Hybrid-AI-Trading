"""
Unit Tests: BreakoutPolygonSignal (Hybrid AI Quant Pro v23.7 Ã¢â‚¬â€œ Hedge-Fund Grade, 100% Coverage)
-----------------------------------------------------------------------------------------------
Covers:
- API key missing
- API/network error
...
def test_api_key_missing(monkeypatch):
    """Guard: POLYGON_KEY missing Ã¢â€ â€™ returns empty list."""
...
def test_api_error_branch(mock_get, signal):
    """Guard: requests.get raises Ã¢â€ â€™ returns empty list."""
...
def test_unexpected_api_format(mock_get, signal):
    """Guard: Polygon returns unexpected JSON Ã¢â€ â€™ returns empty list."""
...
def test_not_enough_bars(signal):
    """Guard: not enough bars Ã¢â€ â€™ HOLD, reason not_enough_bars."""
...
def test_incomplete_data(signal):
    """Guard: some bars missing fields Ã¢â€ â€™ HOLD, reason incomplete_data."""
    bars = [
...
def test_nan_detected(signal):
    """Guard: NaN in closes Ã¢â€ â€™ HOLD, reason nan_detected."""
...
def test_buy_signal(signal):
    """Decision: last close > prev high Ã¢â€ â€™ BUY."""
...
def test_sell_signal(signal):
    """Decision: last close < prev low Ã¢â€ â€™ SELL."""
...
def test_hold_signal(signal):
    """Decision: last close inside prev high/low Ã¢â€ â€™ HOLD."""
...
def test_parse_error(signal):
    """Guard: float conversion fails Ã¢â€ â€™ HOLD, reason parse_error."""
...
def test_outermost_exception(monkeypatch, signal):
    """Guard: _get_polygon_bars raises Ã¢â€ â€™ HOLD, reason exception."""
    monkeypatch.setattr(
        signal,
        "_get_polygon_bars",
...
def test_api_status_code_error(mock_get, signal):
    """Guard: API returns non-200 Ã¢â€ â€™ triggers PolygonAPIError branch."""
    ...
    result = signal._get_polygon_bars("AAPL")
    assert result == []  # exception caught Ã¢â€ â€™ []
