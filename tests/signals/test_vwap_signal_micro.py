"""
VWAP Signal (Hybrid AI Quant Pro v47.2 â€“ Hedge-Fund OE Grade, Full Logging)
---------------------------------------------------------------------------
...
def vwap_signal(
    ...
    try:
        ...
        if len(bars) == 1:
            logger.info("insufficient bars (n=1) â†’ HOLD")
            return "HOLD"
            ...
            try:
                ...
                if (
                    ...
                    if cfg.tie_policy == "SELL":
                        logger.info("symmetry safeguard â†’ SELL")
                        return "SELL"
                    logger.info("symmetry safeguard â†’ HOLD")
                    return "HOLD"
            except Exception as e:
        ...
        if abs(last_close - vwap_val) <= cfg.tolerance:
            if cfg.tie_policy == "SELL":
                logger.info("tie/tolerance â†’ SELL")
                return "SELL"
            logger.info("tie/tolerance â†’ HOLD")
            return "HOLD"
