...
def _disarm_all_gates(te):
    ...
    for name in (
        "validate_symbol","_validate_symbol",
        "is_market_open","is_session_open","session_open","market_open",
        "has_cooldown","is_throttled","has_pending_order","_pre_signal_checks","pre_signal_checks"
    ):
...
def _pos_args(fn):
    ...
    for i,(n,p) in enumerate(sig.parameters.items()):
        ...
        if n in sup: out.append(sup[n]); continue
        # heuristic defaults that wont trip guards
        if "sym" in n or "tick" in n: out.append("AAPL")
        elif "side" in n or "sig" in n: out.append("BUY")
        elif "qty" in n or "size" in n or "amount" in n: out.append(1)
        elif "price" in n or "px" in n: out.append(100.0)
