...
def test_mask_key_local_variants():
    ...
    assert C._mask_key_local("abcdefgh1234").startswith("abcd") and C._mask_key_local("abcdefgh1234").endswith("1234")
...
def test_json_header_but_nonjson_body(monkeypatch):
    ...
    monkeypatch.setattr(_requests,"get", lambda *a,**k: R({"content-type":"application/json"}, data=ValueError("badjson")))
...
def test_text_xml_minimal(monkeypatch):
    ...
    monkeypatch.setattr(_requests,"get", lambda *a,**k: R({"content-type":"text/xml"}, data=None, text=xml))
