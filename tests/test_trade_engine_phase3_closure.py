...
def _positional_args_for(fn):
    ...
    sup = {"symbol":"AAPL","ticker":"AAPL","side":"BUY","signal":"BUY","qty":1,"quantity":1,"size":1,"price":100.0}
    args=[]
    for i,(n,p) in enumerate(sig.parameters.items()):
        if i==0 and n=="self": continue
        if n in sup: args.append(sup[n]); continue
        # heuristic
        if "sym" in n or "tick" in n: args.append("AAPL")
        elif "side" in n or "sig" in n: args.append("BUY")
        elif "qty" in n or "size" in n or "amount" in n: args.append(1)
        elif "price" in n or "px" in n: args.append(100.0)
        else: args.append(None)
    return args

def test_close_remaining_regions_precisely():
    # 241→251: drawdown block executes + continues (no breach) THEN tail normalization
    te = make_engine()
    _max_neutralize(te)
    ...
    # provide direct size and ok/ok submit -> tail (334–339)
    if hasattr(te, "risk_manager"):
        te.risk_manager.approve_trade = lambda *a, **k: {"status":"ok","size":2}
    if hasattr(te, "order_manager"):
        te.order_manager.submit = lambda *a, **k: {"status":"ok","reason":"ok","order_id": 9101}
