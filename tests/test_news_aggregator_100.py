...
class BZ: 
    def get_news(self, s, limit=10, date_from=None):
        return [{"id":"bz1","url":"https://same","title":"META bz","stocks":[{"name":"META","exchange":""}]}]
class PG:
    def __init__(self, calls): self.calls=calls
    def get_news(self, sym, limit=5, date_from=None):
        self.calls.append(limit)
        return [{"id":"pg1","url":"https://same","title":f"{sym} pg","stocks":[{"name":sym,"exchange":""}]}]
class AP: 
    def get_news(self, s, limit=10, date_from=None): return [{"id":"ap1","url":"https://ap","title":"AAPL ap","stocks":[{"name":"AAPL","exchange":""}], "source":"alpaca"}]
class RSS:
    def __init__(self,*a,**k): pass
    def get_news(self, s, date_from=None): return [{"id":"r1","url":"https://r","title":"AAPL r","stocks":[{"name":"AAPL","exchange":""}]}]
...
def test_only_polygon_and_per_symbol_limit(monkeypatch):
    calls=[]
    write_cfg({"aggregate":True,"benzinga":{"enabled":False},"polygon":{"enabled":True},"alpaca":{"enabled":False},"rss":{"enabled":False}})
...
def test_url_first_dedupe_and_source_tagging(monkeypatch):
    write_cfg({"aggregate":True,"benzinga":{"enabled":True},"polygon":{"enabled":True},"alpaca":{"enabled":True},"rss":{"enabled":True}})
