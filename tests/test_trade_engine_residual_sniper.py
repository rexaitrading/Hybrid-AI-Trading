...
def test_residual_snipe(monkeypatch, tmp_path):
    ...
    for poison in (False, True):
        ...
        if poison:
            for a in ("kelly_sizer","performance_tracker"):
                if hasattr(te2, a): setattr(te2, a, None)
        for name in dir(te2):
    ...
    if "smtplib" in sys.modules:
        ...
        try:
            ...
            for name in dir(te):
                if "alert" in name.lower():
                    try: _invoke(getattr(te, name), {"message":"fail2"})
                    except Exception: pass
        finally:
            smtplib.SMTP = good_SMTP

    # 4) Any "audit" writer beyond _write_audit: hit header then exception
    te.audit_log  = str(tmp_path/"audit2.csv"); te.backup_log = str(tmp_path/"backup2.csv")
    for name in dir(te):
        if "audit" in name.lower():
            _invoke(getattr(te, name), defaults)
    te.audit_log  = str(tmp_path/"no_dir"/"audit2.csv"); te.backup_log = str(tmp_path/"no_dir"/"backup2.csv")
    import os
    class Blower:
