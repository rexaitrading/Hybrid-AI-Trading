...
(Hybrid AI Quant Pro v21.0 â€“ Coverage 100%)
------------------------------------------------
Covers the last uncovered branches in trade_engine.py:
- Normalization of {"status": "ok", "reason": "ok"} â†’ "filled"/"normalized_ok"
- Router invalid dict â†’ "invalid_status"
...
def test_sector_exposure_non_breach(engine):
    ...
    # IBM not in tech set â†’ should not breach
...
def test_kelly_sizer_negative_and_exception(engine, monkeypatch):
    ...
    # Negative size â†’ max(1, -5) â†’ size = 1
    ...
    # Exception in KellySizer â†’ fallback size = 1
    monkeypatch.setattr(
        engine.kelly_sizer,
        "size_position",
