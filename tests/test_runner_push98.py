...
def test_load_state_bad_json_and_save_state_exception(monkeypatch):
    ...
    with open(r.state_path, "w", encoding="utf-8") as f: f.write("{bad")
...
def test_pos_load_bad_json(monkeypatch):
    ...
    with open(r.pos_path, "w", encoding="utf-8") as f: f.write("{bad")
...
def test_risk_halt_callsite(monkeypatch):
    ...
    monkeypatch.setattr(RiskManager,"allow_trade", lambda self, **kw: (False,"TRADES_PER_DAY"), raising=False)
    assert r.step() is None
