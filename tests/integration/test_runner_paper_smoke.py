...
@pytest.mark.skipif(os.getenv("IB_TEST_ENABLE") != "1", reason="Set IB_TEST_ENABLE=1 to enable integration tests")
def test_runner_once_smoke(monkeypatch, tmp_path):
    ...
    rc = runner_main(["--config","config/paper_runner.yaml","--once","--mdt","3","--log-file",str(log_file)])
    assert rc == 0