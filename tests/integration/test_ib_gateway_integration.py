...
def test_ib_gateway_handshake_and_quote():
    ...
    with ib_session(host=host, port=port, client_id=client_id, timeout=15, market_data_type=3) as ib:
        ...
        ib.sleep(2.0)
        assert any(v is not None for v in (t.bid, t.ask, t.last)), "No quote fields returned (even delayed)"