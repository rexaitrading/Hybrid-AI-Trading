...
class BrokenSession:
    def query(self, *_): raise Exception("DB fail")
    def close(self): return None

# --- Init / Config ---------------------------------------------------

def test_init_defaults_from_config(monkeypatch):
    monkeypatch.setattr("hybrid_ai_trading.risk.regime_detector.CONFIG", {
        "regime": {
            "enabled": False,
            "method": "simple",
            "lookback_days": 10,
...
def test_detect_bull(monkeypatch):
    ...
    class DummyAbs(pd.Series):
        def sum(self, *_, **__): return 999.0
...
def test_get_prices_db_success(monkeypatch):
    ...
    monkeypatch.setattr("hybrid_ai_trading.risk.regime_detector.SessionLocal", lambda: DummySession(dummy_prices))
    d = RegimeDetector()
    s = d._get_prices("AAPL")
    assert not s.empty

def test_get_prices_db_error(monkeypatch):
    monkeypatch.setattr("hybrid_ai_trading.risk.regime_detector.SessionLocal", lambda: BrokenSession())
    d = RegimeDetector()
    s = d._get_prices("AAPL")
    assert s.empty
...
def test_confidence_all(monkeypatch):
    d = RegimeDetector()
    monkeypatch.setattr(d, "detect", lambda *_: "bull"); assert d.confidence("AAPL") == 0.9
    monkeypatch.setattr(d, "detect", lambda *_: "bear"); assert d.confidence("AAPL") == 0.1
    monkeypatch.setattr(d, "detect", lambda *_: "crisis"); assert d.confidence("AAPL") == 0.3
    monkeypatch.setattr(d, "detect", lambda *_: "transition"); assert d.confidence("AAPL") == 0.5
    monkeypatch.setattr(d, "detect", lambda *_: "sideways"); assert d.confidence("AAPL") == 0.5
    d.enabled = False
...
def test_detect_empty_returns(monkeypatch):
    ...
    assert d.detect("AAPL", prices=[100, 101, 102]) == "sideways"
def test_detect_bull(monkeypatch):
    d = RegimeDetector(min_samples=1, bull_threshold=0.01, bear_threshold=-0.01, crisis_volatility=0.5)
    monkeypatch.setattr(pd.Series, "mean", lambda *_: 0.05)
    monkeypatch.setattr(pd.Series, "std",  lambda *_: 0.01)
    class DummyAbs(pd.Series):
        def sum(self, *_, **__): return 999.0
...
def test_detect_transition(monkeypatch):
    d = RegimeDetector(min_samples=1, bull_threshold=0.02, bear_threshold=-0.02, crisis_volatility=0.5)
    monkeypatch.setattr(pd.Series, "mean", lambda *_: 0.005)
    monkeypatch.setattr(pd.Series, "std",  lambda *_: 0.02)
    class DummyAbs(pd.Series):
        def sum(self, *_, **__): return 999.0
...
def test_detect_normal_flow_without_monkeypatch_closes_and_logs(caplog):
    ...
    assert result in {"bull", "transition", "sideways", "bear", "crisis"}  # must classify something
...
def test_detect_normal_flow_without_monkeypatch_closes_and_logs(monkeypatch, caplog):
    ...
    class DummyAbs(pd.Series):
        def sum(self, *_, **__): return 1.0  # > neutral_tolerance, avoids 'sideways'
    monkeypatch.setattr(pd.Series, "abs", lambda self: DummyAbs([1.0, 1.0]))
...
def test_detect_stats_exception_again_hits_94_96(monkeypatch, caplog):
    ...
    monkeypatch.setattr(pd.Series, "std",  lambda *_: (_ for _ in ()).throw(RuntimeError("std-fail")))
