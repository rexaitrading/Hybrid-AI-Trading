"""
Unit Tests: main.py
(Hybrid AI Quant Pro â€“ Hedge Fund Grade v1.4, 100% Coverage)
============================================================
Covers:
...
- Default argv path (argv=None) uses backtest
...
# ----------------------------------------------------------------------
# Default argv path (argv=None) -> backtest
# ----------------------------------------------------------------------
def test_main_default_no_argv(monkeypatch):
    # Fake pipeline module so default "backtest" import succeeds
    monkeypatch.setitem(
        sys.modules,
        "hybrid_ai_trading.pipelines.backtest",
        types.SimpleNamespace(run_backtest=lambda e: None),
    )
    # With argv=None, argparse reads sys.argv[1:], so set safe argv
    monkeypatch.setattr(sys, "argv", ["python"])
    assert main.main(None) == 0
