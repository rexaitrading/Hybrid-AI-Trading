import types, feedparser as _feed
...
def test_rss_yahoo_and_extra(monkeypatch):
    ...
    parsed1=types.SimpleNamespace(entries=[e1]); parsed2=types.SimpleNamespace(entries=[e2])
    calls={"n":0}
    def parse(url): calls["n"]+=1; return parsed1 if calls["n"]==1 else parsed2
    monkeypatch.setattr(_feed,"parse",parse)
    out=RSSClient(google_template=None,yahoo_template="http://y?s={sym}",extra_feeds=["http://extra"],per_feed_max=5).get_news("AAPL")
