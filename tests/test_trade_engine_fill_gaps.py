...
def test_reset_day_inner_except_branch(monkeypatch):
    ...
    if hasattr(te, "risk_manager"):
        te.risk_manager.reset_day = lambda: (_ for _ in ()).throw(RuntimeError("boom inner"))
...
def test_execute_submit_exception_path(monkeypatch):
    ...
    if hasattr(te, "order_manager"):
        te.order_manager.submit = lambda *a, **k: (_ for _ in ()).throw(RuntimeError("OMS down"))
    for candidate in ("maybe_execute", "try_trade", "place_signal", "run_once", "step_once"):
        if hasattr(te, candidate):
            _safe_call(te, candidate, "AAPL", "BUY", 1)
            break
...
def test_pause_file_gate(monkeypatch):
    ...
    try:
        hit = False
        for candidate in ("check_pause", "_check_pause", "run_once", "step", "loop_tick"):
            if hasattr(te, candidate):
...
def test_finalize_day_exception_path(monkeypatch):
    ...
    for candidate in ("close_day", "finalize_day", "finalize", "end_of_day", "save_state"):
        if hasattr(te, candidate):
            r = _safe_call(te, candidate)
            if isinstance(r, dict):
                assert r.get("status") in {"ok", "error"}
...
def test_reset_day_portfolio_raises_outer_except(monkeypatch):
    ...
    te = make_engine()
    if hasattr(te, "portfolio"):
        te.portfolio.reset_day = lambda: (_ for _ in ()).throw(RuntimeError("portfolio boom"))
    if hasattr(te, "risk_manager"):
        te.risk_manager.reset_day = lambda: {"status": "ok"}
    r = te.reset_day()
    assert isinstance(r, dict)
...
def test_execute_post_submit_status_error_path(monkeypatch):
    ...
    if hasattr(te, "portfolio") and hasattr(te.portfolio, "status"):
        te.portfolio.status = lambda: (_ for _ in ()).throw(RuntimeError("post-submit status fail"))
...
def test_reset_day_both_raise_different(monkeypatch):
    ...
    to exercise multi-except/aggregation paths inside reset_day (175ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“188).
    """
    te = make_engine()
    if hasattr(te, "portfolio"):
        te.portfolio.reset_day = lambda: (_ for _ in ()).throw(ValueError("pf explode"))
    if hasattr(te, "risk_manager"):
        te.risk_manager.reset_day = lambda: (_ for _ in ()).throw(RuntimeError("rk explode"))
    r = te.reset_day()
    assert isinstance(r, dict)
    assert r.get("status") == "error"
...
def test_process_signal_wait_timeout_and_cancel(monkeypatch):
    ...
    to hit 334ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“339 tail branches.
    """
    te = make_engine()
    if hasattr(te, "risk_manager"):
        te.risk_manager.approve_trade = lambda *a, **k: {"status": "ok", "size": 1}
...
def test_reset_day_positions_nonempty_merge(monkeypatch):
    """
    reset_day 175ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“188: exercise the branch that inspects a *non-empty* positions list
...
def test_reset_day_weird_types_but_safe(monkeypatch):
    """
    reset_day 175ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“188: portfolio.reset_day returns list (non-dict), risk reset missing,
...
def test_adaptive_fraction_with_cfg_clamp(monkeypatch):
    """
    adaptive_fraction 205,211ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“212: drive *config-based* clamp (min/max) rather than generic parsing.
    """
    te = make_engine()
...
def test_process_signal_waiter_filled_and_cancel_flag(monkeypatch):
    """
    process_signal 334ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“339: waiter returns 'filled' (fast path), and explicit cancel_on_timeout flags
...
def test_process_signal_drawdown_breach_and_kelly_except(monkeypatch):
    """
    Hits 241â€“246 (drawdown breach) and 256â€“257 (Kelly except -> size=1).
    ...
    te.kelly_sizer = SimpleNamespace(
        size_position=lambda equity, price: (_ for _ in ()).throw(RuntimeError("kelly boom"))
    )
    if hasattr(te, "process_signal"):
        try:
            te.process_signal(
...
def test_ps_sortino_breach_blocked(monkeypatch):
    ...
    if hasattr(te, "_sector_exposure_breach"):
        te._sector_exposure_breach = lambda s: False
    te.config = {"regime": {"enabled": True}, "risk": {"max_drawdown": 0.99, "min_sortino": 10.0}}
