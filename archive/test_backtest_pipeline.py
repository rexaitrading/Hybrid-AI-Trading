...
def test_backtester_runs_with_mock(monkeypatch, tmp_path):
    ...
    assert required_keys.issubset(metrics.keys()), f"Missing metrics: {required_keys - set(metrics.keys())}"

    # --- Assertions: DataFrame integrity ---
    assert not df.empty, "Backtester run() must return a non-empty DataFrame"
    assert {"Strategy", "Symbol", "Sharpe"}.issubset(df.columns), "Missing required DataFrame columns"
    ...
    with open(csv_files[0], "r", encoding="utf-8") as f:
        ...
        expected = {"symbol", "date", "time", "signal", "trade_pnl", "daily_pnl", "cum_equity"}
        assert expected.issubset(headers), f"CSV headers missing: {expected - set(headers)}"
