...
def check_and_exit(stop_pct: float = 0.02,
                   take_pct: float = 0.03,
                   dry_run: bool = True,
                   host: str = "127.0.0.1", port: int = 7497, client_id: int = 2001) -> List[Dict[str,Any]]:
    """
    ...
    for pos in ib.positions():
        ...
        if not last or not avg or avg <= 0:
            actions.append({"symbol": getattr(c,"symbol","?"), "skip": "no price or avgCost"}); continue
        ...
        if reason:
            if dry_run:
                actions.append({"symbol": c.symbol, "qty": qty, "reason": reason, "would_sell": True})
            else:
                t = ib.placeOrder(c, MarketOrder("SELL", qty))
                ib.sleep(1.0)
                actions.append({"symbol": c.symbol, "qty": qty, "reason": reason,
                                "status": t.orderStatus.status, "filled": t.orderStatus.filled,
                                "avgPrice": t.orderStatus.avgFillPrice})
        else:
            actions.append({"symbol": c.symbol, "hold": True,
                            "last": last, "avgCost": avg, "stop": stop_level, "take": take_level})
        time.sleep(0.2)
