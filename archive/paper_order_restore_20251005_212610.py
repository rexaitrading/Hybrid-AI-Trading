...
from ib_insync import IB, Contract, LimitOrder, Stock, StopOrder, TagValue, Trade
...
def rth_open_close(dt: Optional[datetime] = None):
    dt = dt or now_et()
    return dt.replace(hour=9, minute=30, second=0, microsecond=0), dt.replace(
...
def place_bracket(
    ...
    take.ocaType = 1
    take.transmit = False
    take.outsideRth = bool(
        outside_rth
    )  # <-- ensure TP is eligible outside RTH when needed
...
def run(
    ...
    if PAUSE_FLAG.exists():
        print("PAUSE flag present; aborting new orders.")
        return
    ...
    if COOLDOWNS.exists():
        try:
            cooldowns = json.loads(COOLDOWNS.read_text("utf-8"))
        except Exception:
            cooldowns = {}
    key = symbol.upper()
    ...
    if until and now_ts < until:
        ...
        print(f"Cooldown active for {symbol}: {mins} min left; aborting.")
        ib.disconnect()
        return
    ...
    for t in cancelled:
        ...
        print("Cancelled duplicate:", msg)
        _jsonl_write(JSONL_ORDERS, msg)

    # Session guards
    et = now_et()
    if not outside_rth and not is_rth_now(et):
        print("RTH-only and not in RTH; aborting (use --outside-rth 1 to override).")
        ib.disconnect()
        return
    mins = minutes_since_open(et)
    if mins is not None and mins < early_open_block_min:
        print(
            f"Early-open block: {mins} min since open < {early_open_block_min}; aborting."
        )
        ib.disconnect()
        return
    ...
    if spread_bps is not None and not ok:
        print(
            f"Spread too wide: {spread_bps:.2f} bps > cap {spread_bps_cap}; aborting."
        )
        ib.disconnect()
        return
    ...
    if qty is None:
        if not risk_cash:
            raise RuntimeError("--qty not given and --risk-cash not provided")
    ...
    if adaptive:
        ...
        parent_probe.algoParams = [
            TagValue(
                "adaptivePriority",
                "Patient" if adaptive.lower().startswith("p") else "Normal",
            )
        ]
    if whatif:
    ...
    print("Placed:", placed)
    _jsonl_write(JSONL_ORDERS, placed)
    ...
    if autoreprice_sec > 0:
        ...
        if tr_parent.orderStatus.status in ("PreSubmitted", "Submitted"):
            ...
            _jsonl_write(JSONL_ORDERS, placed2)
        ...
        json.dumps(
            {
                **(
                    ...
                    else json.loads(COOLDOWNS.read_text("utf-8") or "{}")
                ),
                symbol.upper(): int(time.time() + cooldown_min * 60),
            },
            ensure_ascii=False,
        ),
        encoding="utf-8",
