...
LOG_TRADES = os.path.join("logs", "trades.jsonl")
    ...
    for row in picked:
        ...
        if should_halt(netliq or 0.0, loss_cap_frac):
            entry = {
                "symbol": sym,
                "halted": True,
                "reason": f"loss_cap {loss_cap_frac:.2%} breached",
                "netliq": netliq,
            }
            results.append(entry)
            with open(LOG_TRADES, "a", encoding="utf-8") as f:
        ...
        # risk budget & sizing
        rc = compute_risk_cap(
            netliq_cad=float(netliq or 0.0),
            daily_target=daily_target,
            allocation=allocation,
        )
        sz = compute_size(
            sym,
            risk_cap_cad=rc["risk_cap_cad"],
        ...
        ok, msg = guardrails(
            {**final_pv},
            max_margin_frac=margin_frac_limit,
            max_qty=max_qty,
            start=start_hm,
            end=end_hm,
        )
        ...
        if ok and sz["qty"] > 0:
            if dry_run:
                ...
                if sz["qty"] >= twap_min_qty:
                    ...
                    send = twap_market(
                        sym, total_qty=sz["qty"], slices=twap_slices, gap_sec=twap_gap
                    )
                else:
...
if __name__ == "__main__":
    ...
    start_hm = (
        os.environ.get("QP_START", "06:00")[:2],
        os.environ.get("QP_START", "06:00")[3:5],
    )
    end_hm = (
        os.environ.get("QP_END", "11:00")[:2],
        os.environ.get("QP_END", "11:00")[3:5],
    )
    ...
    out = run_once(
        ...
        allocation=allocation,
        daily_target=daily_target,
        margin_frac_limit=margin_frac_limit,
        safety_buffer_pct=safety_buffer_pct,
        max_qty=max_qty,
        start_hm=start_hm,
        end_hm=end_hm,
        ...
        twap_min_qty=twap_min_qty,
        twap_slices=twap_slices,
        twap_gap=twap_gap,
        dry_run=dry_run,
        loss_cap_frac=loss_cap_frac,
    )
