...
from hybrid_ai_trading.config.settings import load_config, CONFIG
...
def main():
    ...
    p.add_argument("--price", type=float, default=None, help="Leave blank for market (engine may simulate fill)")
    p.add_argument("--dry-run", action="store_true", help="Force dry_run regardless of config (debug)")
    args = p.parse_args()
