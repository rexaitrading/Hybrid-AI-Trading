...
def call_signal(engine: TradeEngine, symbol: str, signal: str,
                price: float = None, size: int = None, algo: str = None) -> Dict[str,Any]:
    return engine.process_signal(symbol, signal, price=price, size=size, algo=algo)

def find(*args, **kwargs) -> Dict[str,Any]:
    # benign shim; tests that rely on it can override in their fixtures
    return {}
