...
def _rsi(vals, n=14):
    if vals is None or len(vals) < n+1: return None
    gains, losses = [], []
...
def signal_score(symbol: str,
                 ...
                 exchange="SMART", currency="USD", primary="NASDAQ") -> Dict[str, Any]:
    ...
    rsi_score = 0.5; brk_score = 0.5; liq_score = 0.5
    reasons: Dict[str, Any] = {}

    if bars and len(bars) >= 30:
        closes = [b.close for b in bars]
        rsi = _rsi(closes, 14); reasons["rsi"] = rsi
        if rsi is not None:
            if   rsi >= 75: rsi_score = 1.0
            elif rsi >= 65: rsi_score = 0.8
            elif rsi >= 55: rsi_score = 0.6
            elif rsi >= 45: rsi_score = 0.5
            elif rsi >= 35: rsi_score = 0.4
            else:           rsi_score = 0.3

        last = closes[-1]; hh20 = max(closes[-21:-1]); ll20 = min(closes[-21:-1])
        reasons["breakout_ref"] = {"last": last, "hh20": hh20, "ll20": ll20}
        if   last > hh20:            brk_score = 1.0
        elif last > hh20*0.995:      brk_score = 0.75
        elif last > ll20+(hh20-ll20)*0.5: brk_score = 0.55
        else:                        brk_score = 0.35
    else:
    ...
    try:
        tick = ib.reqMktData(c, "", False, False); ib.sleep(0.8)
        ...
        reasons["bid"] = bid; reasons["ask"] = ask
    except Exception:
        reasons["tick"] = "n/a"
