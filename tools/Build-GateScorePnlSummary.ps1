...
foreach ($it in $eventFiles) {
    ...
    if($AllDates){
        $dates = @($events | ForEach-Object { Get-EventDate [CmdletBinding()]
param(
    [ValidateSet("NVDA","SPY","QQQ","ALL")]
    [string]$Symbol = "ALL",
    [switch]$StrictToday
...
exit 0
        ...
        foreach($d in $dates){
            $todayEvents = @()
            foreach($e in $events){ if((Get-EventDate $e) -eq $d){ $todayEvents += $e } }
            if($todayEvents.Count -eq 0){ continue }

            $edgeSourceEvents = $todayEvents
            $pnlSourceEvents  = $todayEvents

            $edges = @()
            $micros = @()
            foreach($e in $edgeSourceEvents){
                $edge  = Get-Num $e @("edge_ratio","mean_edge_ratio","edge","edge_ratio_today")
                $micro = Get-Num $e @("micro_score","mean_micro_score","micro","micro_score_today")
                if($edge -ne $null){ $edges += [double]$edge }
                if($micro -ne $null){ $micros += [double]$micro }
            }

            $meanEdge  = if($edges.Count -gt 0){ ($edges | Measure-Object -Average).Average } else { 0.0 }
            $meanMicro = if($micros.Count -gt 0){ ($micros | Measure-Object -Average).Average } else { 0.0 }

            $rowPnlSamples = Get-EventPnlSamples $todayEvents
            if($rowPnlSamples -le 0){ $rowPnlSamples = [int]$todayEvents.Count }

            $pnlVals = @()
            foreach($e in $pnlSourceEvents){
                $p = Get-Num $e @("realized_pnl","pnl","mean_pnl")
                if($p -ne $null){ $pnlVals += [double]$p }
            }
            $meanPnl = if($pnlVals.Count -gt 0){ ($pnlVals | Measure-Object -Average).Average } else { 0.0 }
