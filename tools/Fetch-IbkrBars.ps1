...
try {
  ...
  # Run fetcher (best-effort)
try {
  $prev = $ErrorActionPreference
  $ErrorActionPreference = "Continue"
  & $py -m hybrid_ai_trading.ib.ib_history_fetch @argv | Out-Host
} catch {
  Write-Host ("[IBKR] FAIL-CLOSED: exception in wrapper: " + [CmdletBinding()]
param(
  [Parameter(Mandatory=$true)][string]$Symbol,
  [Parameter(Mandatory=$true)][string]$AsOfDate,  # YYYY-MM-DD
  [string]$IbHost="127.0.0.1",
...
exit 2.Exception.Message) -ForegroundColor Yellow
} finally {
  $ErrorActionPreference = $prev
}

# Contract: only success if the expected file exists
