...
def main() -> None:
    ...
    for idx, row in enumerate(trades, start=1):
        ...
        result = place_order_phase5(
            ...
            qty=qty,
            price=row.get("price"),
            # For paper-live we treat these as NVDA_BPLUS_LIVE regime trades
            regime="NVDA_BPLUS_LIVE",
        )

        # Flatten the Phase-5 result into the log line so downstream tools
        # (nvda_phase5_paper_to_csv.py) can read ts/symbol/regime/ev/etc.
