"""
Helpers to load Phase-5 JSON configs for QQQ ORB (and others later).

Config JSON is expected at:
- config/phase5/qqq_orb_phase5.json

This module also knows how to overlay EV band information from the
central config/phase5_ev_bands.yml via config.phase5_config_loader.
"""
...
def load_qqq_orb_phase5_config(path: str = "config/phase5/qqq_orb_phase5.json") -> Dict[str, Any]:
    """
    Load the raw QQQ ORB Phase-5 JSON config.

    This is the legacy / baseline loader that existing callers can use.
    """
    cfg_path = Path(path)
    if not cfg_path.exists():
        raise FileNotFoundError(f"QQQ ORB Phase-5 config not found at {cfg_path}")

    with cfg_path.open("r", encoding="utf-8") as f:
        data = json.load(f)

    if not isinstance(data, dict):
        raise ValueError(f"QQQ ORB config at {cfg_path} must be a JSON object (dict)")

    return data


def load_qqq_orb_phase5_config_with_ev(
    path: str = "config/phase5/qqq_orb_phase5.json",
    ev_bands_path: Optional[Path] = None,
) -> Dict[str, Any]:
    """
    Load QQQ ORB Phase-5 config and, if available, enrich it with EV band
    information from config/phase5_ev_bands.yml.

    The YAML is read via config.phase5_config_loader, and we look up the
    EV band for regime key "QQQ_ORB_LIVE" (case-insensitive). If found,
    ...
    """
    cfg = load_qqq_orb_phase5_config(path)
    ...
    try:
        ...
        band = get_ev_band_abs("QQQ_ORB_LIVE", ev_cfg, default=None)
    except Exception:
