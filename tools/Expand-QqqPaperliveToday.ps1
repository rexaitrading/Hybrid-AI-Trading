...
param(
  ...
  [string]$OutPath   = ".\logs\qqq_phase5_paperlive_results_with_micro_today.jsonl",
...
if (-not (Test-Path -LiteralPath $InputPath)) { Write-Error "[QQQ-EXPAND] missing input: $InputPath"; exit 2 }
...
if ($base.Count -lt 1) { Write-Error "[QQQ-EXPAND] no parseable rows in input"; exit 3 }
if ($base.Count -ge $TargetEvents) { Write-Host "[QQQ-EXPAND] already >= TargetEvents ($($base.Count))" -ForegroundColor Green; exit 0 }
...
Write-Host "[QQQ-EXPAND] wrote $($out.Count) rows => $OutPath" -ForegroundColor Green
exit 0
