...
def parse_args(argv=None) -> argparse.Namespace:
    ap = argparse.ArgumentParser("Convert ORB/VWAP NVDA replay summaries to JSONL trades")
    ap.add_argument(
        "--pattern",
        default="orb_vwap_replay_summary_NVDA_*.json",
        help="Glob pattern for NVDA ORB/VWAP summary JSON files (default: orb_vwap_replay_summary_NVDA_*.json)",
    )
    ap.add_argument(
        "--out",
        default=os.path.join("research", "nvda_orb_vwap_replay_trades.jsonl"),
        help="Output JSONL path (default: research/nvda_orb_vwap_replay_trades.jsonl)",
    )
...
def convert_summaries_to_jsonl(pattern: str, out_path: str) -> None:
    ...
    if not files:
        print(f"[NVDA-ORB-JSONL] No files matched pattern: {pattern}")
        return
    ...
    with open(out_path, "w", encoding="utf-8") as out_f:
        for path in files:
            ...
            try:
                ...
                print(f"[NVDA-ORB-JSONL] ERROR reading {path}: {exc}")
                continue
    ...
    print(f"[NVDA-ORB-JSONL] Processed {n_files} summary files.")
    print(f"[NVDA-ORB-JSONL] Emitted {n_trades} trade record(s) to {out_path}")
