# tools/stream.ps1  clean helpers (PS5.1-safe)
...
function start-stream {
  ...
  # Guard: do not spawn duplicates
  $existing = Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
    Where-Object { # tools/stream.ps1  clean helpers (PS5.1-safe)
...
function start-stream {
  ...
  # Launch runner (no console freeze; logs capture)
...
}.CommandLine -like '*runner_stream.py*' }
  if ($existing) {
    Write-Host "[STREAM] Already running. Skipping new launch." -ForegroundColor Yellow
    return
}

# Purge: kill any system-python runner_stream (only allow venv python)
...
function status-stream {
  ...
  Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
    Where-Object { $_.CommandLine -like '*runner_stream.py*' } |
    Select-Object ProcessId, CommandLine | Format-Table -AutoSize
}.CommandLine -match 'runner_stream\.py' -and
    # tools/stream.ps1  clean helpers (PS5.1-safe)
...
function status-stream {
  ...
  Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
    Where-Object { $_.CommandLine -like '*runner_stream.py*' } |
    Select-Object ProcessId, CommandLine | Format-Table -AutoSize
}.CommandLine -notmatch [regex]::Escape($Script:VenvPy)
  }
if ($bad) {
  foreach($b in $bad){
    try {
