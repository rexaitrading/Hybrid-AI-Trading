...
def convert_qqq_phase5_jsonl_to_csv(src: Path, dst: Path) -> int:
    """
    Convert QQQ Phase-5 paper-live JSONL into a flat CSV for Notion.
    """
...
def main() -> None:
    src = Path("logs") / "qqq_phase5_paperlive_results.jsonl"
    dst = Path("logs") / "qqq_phase5_paper_for_notion.csv"
    count = convert_qqq_phase5_jsonl_to_csv(src, dst)
