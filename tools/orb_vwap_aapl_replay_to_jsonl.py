...
def parse_args(argv=None) -> argparse.Namespace:
    ap = argparse.ArgumentParser("Convert ORB/VWAP AAPL replay summaries to JSONL trades")
    ap.add_argument(
        "--pattern",
        default="orb_vwap_replay_summary_AAPL_*.json",
        help="Glob pattern for AAPL ORB/VWAP summary JSON files (default: orb_vwap_replay_summary_AAPL_*.json)",
    )
    ap.add_argument(
        "--out",
        default=os.path.join("research", "aapl_orb_vwap_replay_trades.jsonl"),
        help="Output JSONL path (default: research/aapl_orb_vwap_replay_trades.jsonl)",
    )
...
def convert_summaries_to_jsonl(pattern: str, out_path: str) -> None:
    ...
    if not files:
        print(f"[AAPL-ORB-JSONL] No files matched pattern: {pattern}")
        return
    ...
    with open(out_path, "w", encoding="utf-8") as out_f:
        for path in files:
            ...
            try:
                ...
                print(f"[AAPL-ORB-JSONL] ERROR reading {path}: {exc}")
                continue

            symbol = summary.get("symbol")
            has_signal = bool(summary.get("has_signal", False))

            # Skip days with no signal
            if not has_signal:
                continue
    ...
    print(f"[AAPL-ORB-JSONL] Processed {n_files} summary files.")
    print(f"[AAPL-ORB-JSONL] Emitted {n_trades} trade record(s) to {out_path}")
