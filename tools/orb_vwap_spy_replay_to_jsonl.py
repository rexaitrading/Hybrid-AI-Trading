...
def parse_args(argv=None) -> argparse.Namespace:
    ap = argparse.ArgumentParser("Convert ORB/VWAP SPY replay summaries to JSONL trades")
    ap.add_argument(
        "--pattern",
        default="orb_vwap_replay_summary_SPY_*.json",
        help="Glob pattern for SPY ORB/VWAP summary JSON files (default: orb_vwap_replay_summary_SPY_*.json)",
    )
    ap.add_argument(
        "--out",
        default=os.path.join("research", "spy_orb_vwap_replay_trades.jsonl"),
        help="Output JSONL path (default: research/spy_orb_vwap_replay_trades.jsonl)",
    )
...
def convert_summaries_to_jsonl(pattern: str, out_path: str) -> None:
    ...
    if not files:
        print(f"[SPY-ORB-JSONL] No files matched pattern: {pattern}")
        return
    ...
    with open(out_path, "w", encoding="utf-8") as out_f:
        for path in files:
            ...
            try:
                ...
                print(f"[SPY-ORB-JSONL] ERROR reading {path}: {exc}")
                continue
    ...
    print(f"[SPY-ORB-JSONL] Processed {n_files} summary files.")
    print(f"[SPY-ORB-JSONL] Emitted {n_trades} trade record(s) to {out_path}")
