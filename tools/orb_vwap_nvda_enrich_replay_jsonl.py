...
def parse_args(argv=None) -> argparse.Namespace:
    ap = argparse.ArgumentParser("Enrich NVDA ORB/VWAP trades JSONL with PnL and EV using bar data")
    ap.add_argument(
        "--jsonl",
        default=os.path.join("research", "nvda_orb_vwap_replay_trades.jsonl"),
        help="Input JSONL of NVDA ORB/VWAP trades (default: research/nvda_orb_vwap_replay_trades.jsonl)",
    )
    ap.add_argument(
        "--bars",
        default=os.path.join("data", "NVDA_1m.csv"),
        help="NVDA 1m bar CSV (default: data/NVDA_1m.csv)",
    )
    ap.add_argument(
        "--out",
        default=os.path.join("research", "nvda_orb_vwap_replay_trades_enriched.jsonl"),
        help="Output enriched JSONL (default: research/nvda_orb_vwap_replay_trades_enriched.jsonl)",
    )
...
def _find_timestamp_column(df: pd.DataFrame) -> str:
    ...
    for col in df.columns:
        if "time" in col.lower() or "date" in col.lower():
            return col
    raise ValueError(
        f"Could not find a timestamp column in NVDA bars. "
...
def enrich_jsonl(jsonl_path: str, bars_path: str, out_path: str) -> None:
    ...
    if not os.path.exists(bars_path):
        raise FileNotFoundError(f"NVDA bars CSV not found: {bars_path}")

    print(f"[NVDA-ENRICH] Loading bars from {bars_path} ...")
    bars_df = pd.read_csv(bars_path)
    ts_col = _find_timestamp_column(bars_df)
    if "close" not in bars_df.columns:
        raise ValueError(f"[NVDA-ENRICH] Expected 'close' column in bars CSV, got columns={list(bars_df.columns)}")
    ...
    print(f"[NVDA-ENRICH] Reading trades from {jsonl_path} ...")
    with open(jsonl_path, "r", encoding="utf-8") as src, open(out_path, "w", encoding="utf-8") as dst:
        for line in src:
            ...
            try:
                ...
                print(f"[NVDA-ENRICH] ERROR parsing JSONL line {n_in}: {exc}")
                n_failed += 1
                continue
            ...
            try:
                ...
                print(f"[NVDA-ENRICH] ERROR parsing timestamps for line {n_in}: {exc}")
                dst.write(json.dumps(rec, ensure_ascii=False) + "\n")
                n_out += 1
                n_failed += 1
                continue
            ...
            if session_bars.empty:
                print(f"[NVDA-ENRICH] No bars found for session date {session_date.date()} (line {n_in})")
                dst.write(json.dumps(rec, ensure_ascii=False) + "\n")
                n_out += 1
                n_failed += 1
                continue
            ...
            if entry_match.empty:
                print(f"[NVDA-ENRICH] Could not find entry bar at/after {entry_ts} in session {session_date.date()} (line {n_in})")
                dst.write(json.dumps(rec, ensure_ascii=False) + "\n")
                n_out += 1
                n_failed += 1
                continue
    ...
    print(f"[NVDA-ENRICH] Read {n_in} trade record(s) from {jsonl_path}")
    print(f"[NVDA-ENRICH] Wrote {n_out} enriched record(s) to {out_path}")
    if n_failed:
        print(f"[NVDA-ENRICH] {n_failed} record(s) failed enrichment; original fields were preserved for those.")
