...
function Get-SessionDate([string]$sym){
  ...
  $r = $rows | Where-Object { [CmdletBinding()]
param(
  [int]$TimeoutSec = 60,
  [ValidateSet("NVDA","SPY","QQQ","ALL")]
  [string]$Symbol = "ALL"
...
if($Symbol -ne "ALL"){
  if(-not (Has-TodayRow -sym $Symbol)){
    Write-Host ("[GS-BUILD] FAIL-CLOSED: no session row for {0} in {1} today={2}" -f $Symbol,$csv,$today) -ForegroundColor Yellow
    exit 2
  }
}
...
foreach($s in $syms){
  if(-not (Has-TodayRow -sym $s)){
    Write-Host ("[GS-BUILD] FAIL-CLOSED: no session row for {0} in {1} today={2}" -f $s,$csv,$today) -ForegroundColor Yellow
    exit 2
...
.symbol -eq $sym } | Sort-Object as_of_date -Descending | Select-Object -First 1
  if (-not $r) { return "" }
