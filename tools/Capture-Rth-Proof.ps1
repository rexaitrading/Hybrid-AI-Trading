...
$transcript = Join-Path $root ("logs\{0}\rth_proof_capture_{1}.txt" -f $Market,$ts)
...
try {
  ...
  "=== (2) ProducersOnly OneTap ===" | Out-Host
  .\tools\PreMarket-OneTap.ps1 -ProducersOnly -Symbol $Symbol | Out-Host

  "=== (3) BlockG build ===" | Out-Host
  .\tools\Build-BlockGStatusStub.ps1 -Market $Market -Symbol $Symbol | Out-Host

  $bgp = Join-Path $root ("logs\{0}\blockg_status_stub.json" -f $Market)
  ...
  "=== PROOF FIELDS (market=" + $Market + ") ===" | Out-Host
  $bg | Select-Object `
