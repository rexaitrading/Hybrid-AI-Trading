...
def compute_soft_veto_ev_fields(ev: float, realized_pnl: float) -> Dict[str, Any]:
    """
    Phase-5 QQQ soft EV veto diagnostics.
...
def append_qqq_phase5_paper_entry(
    ...
    This is QQQ-specific and *does not* change any trade gating behavior.
    """
...
def main() -> None:
    ...
    # Append QQQ-specific soft EV diagnostics (diagnostic only).
    try:
        ...
        print("\n[PHASE5/QQQ] Appended soft EV diagnostic entry to", QQQ_PAPER_JSONL_PATH)
    except Exception as e:
        print("\n[WARN] Failed to append QQQ soft EV diagnostic entry:", e)
