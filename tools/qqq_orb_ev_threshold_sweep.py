...
def parse_args(argv=None) -> argparse.Namespace:
    ap = argparse.ArgumentParser("QQQ ORB/VWAP EV vs GateScore edge_ratio threshold sweep")
    ap.add_argument(
        "--jsonl",
        default=os.path.join("research", "qqq_orb_vwap_replay_trades_enriched.jsonl"),
        help="Input enriched JSONL (default: research/qqq_orb_vwap_replay_trades_enriched.jsonl)",
    )
    ap.add_argument(
        "--start",
        type=float,
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                print(f"[QQQ-THR] ERROR parsing line: {exc}")
    return trades
...
def sweep(trades: List[Dict[str, Any]], start: float, stop: float, step: float, min_pnl_pct: float, max_cost_bp: float) -> None:
    if not trades:
        print("[QQQ-THR] No trades to analyze.")
        return

    print("[QQQ-THR] Total trades:", len(trades))
    print("[QQQ-THR] Sweep edge_ratio from", start, "to", stop, "step", step)
    print("[QQQ-THR] Filters: min_pnl_pct>=", min_pnl_pct, "max_cost_bp<=", max_cost_bp)
    print()
