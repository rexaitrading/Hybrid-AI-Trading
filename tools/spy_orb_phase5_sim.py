"""
Simple SPY ORB (Opening Range Breakout) simulator for 1-minute OHLCV data.
...
- ORB window: first 15 minutes of RTH (09:30 - 09:44 inclusive).
...
- TP = entry_price + 2 * risk_per_share (approx +2R).
- If both TP and SL hit in same bar, SL is assumed first (conservative).
...
class Trade:
    ...
    tp_price: float
    sl_price: float
    outcome: str  # "TP", "SL", "EOD", "NO_TRADE"
    r_multiple: float
    gross_pnl_pct: float
    bars_held: int
    session: str
...
def simulate_orb_for_day(
    ...
    # ORB window: first orb_minutes bars of RTH
    orb_window_bars = []
    for b in bars:
    ...
    # If still in trade at end-of-session, close at last bar close
...
def main() -> None:
    ap = argparse.ArgumentParser(description="SPY ORB 1m simulator (JSONL trades).")
    ap.add_argument("--csv", required=True, help="Path to SPY 1m OHLCV CSV.")
    ap.add_argument("--date", required=True, help="Trading date (YYYY-MM-DD).")
    ap.add_argument("--symbol", default="SPY", help="Symbol label (default SPY).")
    ap.add_argument("--out", required=True, help="Output JSONL path.")
    ap.add_argument("--orb-minutes", type=int, default=15, help="ORB window length in minutes (default 15).")
    ap.add_argument("--tp-r", type=float, default=2.0, help="Take-profit in R multiples (default 2.0).")
    args = ap.parse_args()

    bars = load_bars_for_date(args.csv, args.date)
    if not bars:
        print(f"[SPY_ORB] No bars found for date {args.date} in {args.csv}")
    ...
    print(f"[SPY_ORB] Wrote {len(trades)} trades to {args.out}")
