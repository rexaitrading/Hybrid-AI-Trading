...
param(
  ...
  [string]$StampPath  = ".\logs\spy_live_ready_stamp.json",
  [string]$OutPath    = ".\logs\notion\spy_live_allowed_payload.json"
)
...
try {
  ...
  Write-Host ("[NOTION] WARN: missing stamp -> fail-closed spy_live_allowed=false :: " + $_.Exception.Message) -ForegroundColor Yellow
  $st = [pscustomobject]@{
    as_of_date = $today
    spy_live_ready = $false
...
$out = [ordered]@{
  ...
  reasons_not_ready = (
    @(
      if(-not (([bool]$st.spy_live_ready -and [bool]$bg.spy_blockg_ready))){"spy_live_allowed=false"}
    ) +
