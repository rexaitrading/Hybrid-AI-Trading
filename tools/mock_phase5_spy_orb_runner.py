# --- SPY ORB Phase-5 R-based config (autogenerated) ----------------------
...
"""
Mock SPY ORB Phase-5 + Risk + no-averaging-down runner.

- Reads trades from logs/spy_phase5_replay_gated.jsonl
...
- Writes enriched records to logs/spy_phase5_orb_phase5_risk.jsonl
...
INPUT_JSONL = LOGS_DIR / "spy_phase5_replay_gated.jsonl"
OUTPUT_JSONL = LOGS_DIR / "spy_phase5_orb_phase5_risk.jsonl"
...
def main() -> None:
    print("[PHASE5_SPY] Loading SPY ORB replay trades from JSONL...")
    trades = load_trades(INPUT_JSONL)
    print(f"[PHASE5_SPY] Loaded {len(trades)} trades from {INPUT_JSONL.name}")
    ...
    print(
        f"Summary (COMBINED): symbol=SPY total_trades={len(enriched)} "
