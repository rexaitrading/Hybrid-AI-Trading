...
param(
  ...
  # Correct named parameter binding for .ps1 tools:
  [hashtable]$Params = @{} ,

  # Optional env (transport only)
  [hashtable]$EnvSet = @{} ,
...
$tmp = Join-Path $env:TEMP ("hat_guard3_" + (Get-Date).ToString("yyyyMMdd_HHmmss") + ".ps1")

$clearPins = @(
  "HAT_ASOF_DATE","HAT_AS_OF_DATE",
  "HAT_MARKET","HAT_SYMBOL","HAT_SESSION_NAME",
  "HAT_LOGS_DIR","HAT_LOGS_DIR_OUT",
  "HAT_MODE","HAT_REPO_ROOT","HAT_IS_PAPER"
)
...
# Build Params hashtable literal in child
$lines.Add('$__params = @{}')
if($Params -and $Params.Count -gt 0){
  foreach($k in @($Params.Keys)){
    ...
    if($val -eq $null){ continue }
    $v = ([string]$val).Replace('"','""')
    $lines.Add(('$__params["{0}"] = "{1}"' -f $name, $v))
  }
}

$lines.Add(('$__tool = "{0}"' -f $toolPath.Replace('"','""')))
$lines.Add('& $__tool @__params *>&1 | Out-Host')
