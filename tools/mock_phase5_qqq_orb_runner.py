"""
Mock QQQ ORB Phase-5 + Risk + no-averaging-down runner.

- Reads trades from logs/qqq_phase5_replay_gated.jsonl
...
- Writes enriched records to logs/qqq_phase5_orb_phase5_risk.jsonl
...
INPUT_JSONL = LOGS_DIR / "qqq_phase5_replay_gated.jsonl"
OUTPUT_JSONL = LOGS_DIR / "qqq_phase5_orb_phase5_risk.jsonl"
...
def main() -> None:
    print("[PHASE5_QQQ] Loading QQQ ORB replay trades from JSONL...")
    trades = load_trades(INPUT_JSONL)
    print(f"[PHASE5_QQQ] Loaded {len(trades)} trades from {INPUT_JSONL.name}")
    ...
    for trade in trades:
        ...
        regime = trade.get("regime", "QQQ_ORB_REPLAY")
        ts = trade.get("entry_ts") or trade.get("ts")

        # Existing Phase-5 decision from pnl_gate
        base_phase5_allowed = bool(trade.get("phase5_allowed", True))
        base_phase5_reason = trade.get("phase5_reason", "pnl_gate_unknown")
    ...
    print(
        f"Summary (COMBINED): symbol=QQQ total_trades={len(enriched)} "
...
# --- Phase-5 bridge-backed no-averaging helper (QQQ ORB replay) ---
...
def phase5_no_averaging_down_for_symbol_bridge(
    ...
    This is a temporary inline helper for the QQQ ORB replay runner.
    ...
    symbol = str(trade.get("symbol", "QQQ"))
