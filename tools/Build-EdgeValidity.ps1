...
try {
  if($rc -and ($rc.PSObject.Properties.Name -contains "market_closed_today") -and [bool]$rc.market_closed_today){
    ...
    # keep reasons[] but stamp a clear reason string if present
...
$proxy = Has-ProxyMetricsToday -LogsDir $logsDir -TodayLocal $todayLocal

if($allow -and $evidenceRows -gt 0 -and (-not $proxy)){
  $obj.ok_today = $true
  $obj.edge_score = 0.0
  ...
  if($proxy){ $rs += "deny_proxy_metrics_source" }
...
Write-Host ("[EDGE] wrote " + $outPath + " ok_today=" + ([bool]$obj.ok_today) + " reasons=" + ((@($obj.reasons) -join ","))) -ForegroundColor Yellow
