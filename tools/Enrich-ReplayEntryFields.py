...
def enrich_trade(
    ...
    if entry_px_f <= 0.0:
        entry_px_f = float(default_entry)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Enrich replay JSONL with entry_px and qty fields."
    )
    parser.add_argument(
        "--jsonl",
        required=True,
    ...
    parser.add_argument(
        ...
        type=float,
        default=100.0,
        help="Default entry price if none can be inferred.",
    )
    parser.add_argument(
        "--default-qty",
        type=float,
        default=100.0,
        help="Default quantity if none can be inferred.",
    )
    ...
    print("[ENRICH-GEN] Input JSONL:", jsonl_path)
    if not jsonl_path.exists():
        print("[ENRICH-GEN] ERROR: JSONL file not found.")
        return
    ...
    with jsonl_path.open("r", encoding="utf-8") as fin, tmp_path.open(
        ...
        for line in fin:
            ...
            try:
                ...
                print("[ENRICH-GEN] WARN: Skipping invalid JSON line.")
                continue

            obj = enrich_trade(obj, args.default_entry, args.default_qty)
    ...
    print(f"[ENRICH-GEN] Done. In={n_in}, Out={n_out}. Updated entry_px and qty.")
