...
def main() -> None:
    ...
    print("[AAPL-PHASE5-SIM] Loaded thresholds:", thresholds)
    ...
    print(f"[AAPL-PHASE5-SIM] Loaded {len(trades)} enriched trade(s) from {jsonl_path}")

    # 3) Gate signals via orb_vwap_gatescore_filter logic
    gated = filter_trades(trades, thresholds)
    print(f"[AAPL-PHASE5-SIM] {len(gated)} trade(s) passed GateScore + cost filters.")

    if not gated:
        print("[AAPL-PHASE5-SIM] No trades passed the gate; nothing to simulate.")
        return

    # 4) Wire in TradeEnginePhase5: consider_add() before simulated add
    engine = TradeEnginePhase5()
    print("[AAPL-PHASE5-SIM] Using RiskConfig:", engine.risk_manager.risk_cfg)
    print("[AAPL-PHASE5-SIM] Using CostConfig:", engine.risk_manager.cost_cfg)
    ...
    print("[AAPL-PHASE5-SIM] Final simulated notional:", current_notional)
