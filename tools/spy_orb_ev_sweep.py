...
Each line is an ORB trade with fields like:
    symbol, side, outcome, r_multiple, gross_pnl_pct,
    day, orb_minutes, tp_pct, source_file, ...

We compute EV / stats grouped by (orb_minutes, tp_pct):

    - n_trades
    - win_rate (TP / total, or r_multiple > 0)
    - avg_r
    - ev_r (same as avg_r)
    - avg_gross_pnl_pct

Output:
    research/spy_orb_ev_threshold_sweep.csv
"""
...
def sweep_ev(input_path: Path, output_path: Path) -> None:
    if not input_path.exists():
        print(f"[spy_orb_ev] Input file not found: {input_path}")
        return
    ...
    with input_path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            # Skip records where we didn't parse metadata
            ...
            if r is None:
                # Fall back to gross_pnl_pct if needed
            ...
            # Define win as r_multiple > 0 (or outcome == "TP")
            outcome = rec.get("outcome")
            if outcome == "TP" or r_val > 0:
                wins[key] += 1

    if not counts:
        print("[spy_orb_ev] No trades with orb_minutes+tp_pct metadata found.")
        return
    ...
    with output_path.open("w", newline="", encoding="utf-8") as csv_f:
        ...
        # sort by orb_minutes then tp_pct for readability
    ...
    print(f"[spy_orb_ev] Wrote EV sweep to {output_path}")


def main() -> None:
    parser = argparse.ArgumentParser(description="SPY ORB EV sweep")
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--output",
        default="research/spy_orb_ev_threshold_sweep.csv",
