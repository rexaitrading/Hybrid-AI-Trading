"""
PaperLiveWithoutIBG QQQ Phase-5 runner.

- NO broker / NO IBG; this is a pure in-process paper runner.
- Reads QQQ candidate trades from logs/paper_trades.jsonl.
- For each QQQ row, calls place_order_phase5(engine, ...) to apply:
  - Phase-5 decisions gate (from qqq_phase5_decisions.json),
  - RiskManager.phase5_no_averaging_down_for_symbol (if implemented).
- Logs decisions/results to logs/qqq_phase5_paperlive_results.jsonl.
"""
...
class PaperEngine:
    """
    Minimal in-process engine for paper trading without IBG (QQQ).
    ...
    def place_order(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        Expected kwargs at least:
            symbol, side, qty
        """
        symbol = str(kwargs.get("symbol") or kwargs.get("sym") or "QQQ")
...
def load_qqq_paper_trades() -> List[Dict[str, Any]]:
    """
    Load QQQ rows from logs/paper_trades.jsonl and sort by ts ascending.
    """
...
def main() -> None:
    engine = PaperEngine()
    trades = load_qqq_paper_trades()

    dst = Path("logs") / "qqq_phase5_paperlive_results.jsonl"
    out_f = dst.open("w", encoding="utf-8")

    print(f"Loaded {len(trades)} QQQ paper trade candidates.")
    ...
    out_f.close()
    print(f"Wrote QQQ Phase-5 paper-live results to {dst}")
