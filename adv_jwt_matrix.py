...
AUDS = [None, "api.coinbase.com"]            # try no aud and host aud
URIS = [f"{METHOD} {PATH}",
        f"{METHOD} {HOST}{PATH}",
        f"{METHOD} https://{HOST}{PATH}"]    # try path, host+path, scheme+host+path

def build_jwt(kid, aud, uri):
    now=int(time.time())
    header={"alg":"ES256","typ":"JWT"}
    if kid: header["kid"]=kid
    claims={"iss":API_KEY,"sub":API_KEY,"nbf":now-5,"iat":now,"exp":now+110,"uri":uri}
    ...
    for aud in AUDS:
        for uri in URIS:
            try:
                ...
                r=requests.get(f"https://{HOST}{PATH}", headers={"Authorization":"Bearer "+jwt}, timeout=20)
                ok=200<=r.status_code<300
                rec={"kid":kid or "<none>","aud":aud or "<none>","uri":uri,"status":r.status_code,"ok":ok,"body":r.text[:250]}
                results.append(rec)
                if ok:
                ...
                results.append({"kid":kid or "<none>","aud":aud or "<none>","uri":uri,"status":None,"ok":False,"err":repr(e)})
