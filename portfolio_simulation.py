...
print("📄 Config loaded:", TICKERS, START_DATE, END_DATE, f"Rebalance={REBALANCE}, RF={RISK_FREE}")
...
def fetch_crypto(symbol: str, start_date: str) -> pd.DataFrame:
    try:
        ...
        ohlcv = _binance.fetch_ohlcv(
            symbol, timeframe="1d",
            since=_binance.parse8601(start_date + "T00:00:00Z")
        )
...
def simulate_portfolio(weights, returns_df, rebalance="daily"):
    ...
    return {
        "Ann_Return": ann_return, "Ann_Volatility": ann_vol, "Sharpe": sharpe,
        "Sortino": sortino, "Calmar": calmar, "VaR_95": var95,
        "CVaR_95": cvar95, "Max_Drawdown": max_dd,
        "Cumulative": port_cum, "DailyReturns": daily_returns
    }
...
if __name__ == "__main__":
    ...
    print("⭐ Optimal Weights:", dict(zip(combined.columns, [round(w*100,2) for w in opt_w])))
    print("📊 Optimal Metrics:", {k: round(v,4) for k,v in opt_metrics.items() if isinstance(v,(int,float,np.floating))})
