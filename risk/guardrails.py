...
def in_trading_window(start_hm=("06","00"), end_hm=("11","00")) -> bool:
    now = _now_pt().timetz()
    sh, sm = map(int, start_hm); eh, em = map(int, end_hm)
...
def guardrails(preview: Dict[str, Any],
               max_margin_frac: float = 0.20,
               max_qty: int = 1000,
               start=("06","00"), end=("11","00")) -> Tuple[bool,str]:
    ...
    if not in_trading_window(start, end):
        return False, f"outside trading window PT {start[0]}:{start[1]}–{end[0]}:{end[1]}"
