...
def main():
    ...
    try:
        ...
        if args.json:
            ...
            print(
                f"[{now}] connected={out['connected']} serverVersion={out['serverVersion']} mdType={out['mdType']}"
            )
            for q in out["quotes"]:
                print(
                    f"{q['symbol']:>6}  bid={q['bid']}  ask={q['ask']}  last={q['last']}  close={q['close']}  vol={q['volume']}"
                )
            if acct:
                print(
                    "Account:",
