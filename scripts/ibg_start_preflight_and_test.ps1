...
function Wait-ForIBG([int]$totalSec,[int]$needed){
  ...
  while((Get-Date) -lt $deadline){
    ...
    if($tick % 7 -eq 0){ Write-Host ("Ã¢â‚¬Â¦waiting for login & API (sec ~{0})" -f ($tick*0.7)) -ForegroundColor DarkGray }
...
Start-Process $Exe
Write-Host "Log in to Paper; waiting for API listener (4002) to be stably owned by ibgatewayÃ¢â‚¬Â¦" -ForegroundColor Yellow
...
Write-Host "Listener is up and stable. Small settleÃ¢â‚¬Â¦" -ForegroundColor Green
Start-Sleep -Seconds 6

# re-check just before handshake
$tcp = Get-NetTCPConnection -State Listen -LocalPort 4002 -ErrorAction SilentlyContinue | Select-Object -First 1
if(-not $tcp){ Write-Host "Listener dropped; waiting againÃ¢â‚¬Â¦" -ForegroundColor Yellow; if (-not (Wait-ForIBG -totalSec 90 -needed $ConsecOK)) { exit 0 } }
