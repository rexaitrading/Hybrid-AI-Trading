...
def _pass(msg):  global passed; passed += 1; ok(msg)
def _fail(msg):  global failed; failed += 1; err(msg)
def _warn(msg):  global warned; warned += 1; warn(msg)
...
if 0.5 <= threshold <= 0.7 and 0.1 <= neutral <= 0.35 and 1 <= smooth <= 5:
    _pass(f"YAML: sentiment_filter ok (threshold={threshold}, neutral={neutral}, smoothing={smooth})")
else:
    _fail(f"YAML: sentiment_filter unexpected (threshold={threshold}, neutral={neutral}, smoothing={smooth})")
...
try:
    ...
    if not connected:
        ...
        tags = {"NetLiquidation": None, "AvailableFunds": None, "BuyingPower": None, "CashBalance": None}
        for item in ib.accountSummary():
            if item.tag in tags: tags[item.tag] = item.value
        if all(tags.values()):
