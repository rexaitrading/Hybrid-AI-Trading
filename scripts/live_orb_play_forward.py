...
def _call_run_replay(*, df, symbol, mode, qty, fees, orb_minutes, force_exit, notion):
    ...
    try:
        ...
        try:
            return run_replay(df=df, symbol=symbol, mode=mode,
                              orb_minutes=orb_minutes, force_exit=force_exit, notion=notion)
...
def qualify(symbol, primary):
    c = Stock(symbol, "SMART", "USD", primaryExchange=primary) if primary else Stock(symbol, "SMART", "USD")
...
def seed_history(contract):
    attempts = [
        dict(durationStr=f"{DUR_SEC} S", barSizeSetting="1 min", whatToShow="TRADES", useRTH=USE_RTH),
    ...
    for i, kw in enumerate(attempts, 1):
        try:
            ...
            bars = ib.reqHistoricalData(contract, endDateTime="", formatDate=1, keepUpToDate=False, **kw)
            if bars:
                df = pd.DataFrame([{
                    "timestamp": b.date if isinstance(b.date, str) else b.date.strftime("%Y-%m-%d %H:%M:%S"),
...
class _RT:
    ...
    def __init__(self, t,o,h,l,c,v,w=0.0,count=1):
        self.time=t; self.open=o; self.high=h; self.low=l; self.close=c; self.volume=v; self.wap=w; self.count=count
...
def resample_5s_to_1m(df):
    ...
    ohlc = x[["open","high","low","close"]].resample("1min", label="right", closed="right").agg(
...
def _simulate_from_seed(seed_df, *, state, pace_sec=0.10):
    ...
    for i in range(1, n+1):
        ...
        b = _RT(t, r["open"], r["high"], r["low"], r["close"], int(r["volume"]), w=r["close"], count=1)
...
def on_rt_bar(rtbar, *, state):
    ...
    if len(tail) >= ORB_MIN + 5:
        ...
        res = _call_run_replay(df=tail.copy(), symbol=SYMBOL, mode=MODE,
                               qty=QTY, fees=FEES, orb_minutes=ORB_MIN,
...
def main():
    ...
    if os.getenv("LIVE_SIM", "0") in ("1","true","True","YES","yes"):
        state = {"five_s": five_s}
        print("[sim] LIVE_SIM=1 -> using synthetic minute ticks from seed (no RT subscription)", flush=True)
