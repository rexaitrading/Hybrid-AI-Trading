...
rm = RiskManager(RiskConfig(day_loss_cap_pct=0.02, max_drawdown_pct=0.08,
                            max_trades_per_day=2, max_consecutive_losers=1,
                            cooldown_bars=1))
...
# 2) Max drawdown
header("max drawdown")
clear_halts(); clear_daily(); clear_trades()
rm._state["rolling_peak_equity"] = 100.0
...
ok, reason = rm.allow_trade(notional=100, side="BUY", bar_ts=now+3 + rm.cfg.cooldown_bars*3_600_000 + 1)
print("after cooldown:", ok, reason)

# 4) Trades/day limit
header("max trades per day")
clear_halts(); clear_daily(); clear_drawdown()
