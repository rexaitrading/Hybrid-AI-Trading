...
if ($qty -lt 1) {
  Write-Host ("Order exceeds CAD value cap at this price. " +
              "MaxOrderValueCAD={0}, USDCAD={1:N4}, lmt={2:N2} Ã¢â€ â€™ qtyValue={3}" -f $MaxOrderValueCAD,$usdcad,$lmt,$qtyValue)
  Write-Host "Tip: lower -RiskCash OR raise -MaxOrderValueCAD in script OR TWS Presets."
  exit 0
}
if ($qtyRisk -gt $MaxQty)   { Write-Host "Clamped by Size Limit: $qtyRisk Ã¢â€ â€™ $qtySize (max $MaxQty)"; }
if ($qtySize -gt $qty)      { Write-Host "Clamped by Value Limit: $qtySize Ã¢â€ â€™ $qty (max CAD $MaxOrderValueCAD)"; }
