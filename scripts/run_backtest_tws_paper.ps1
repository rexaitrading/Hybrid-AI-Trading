...
if (-not $Backtest) {
  Write-Host "Ã°Å¸â€Å½ Scanning repo for backtest.py ..." -ForegroundColor DarkYellow
  $found = Get-ChildItem $RepoRoot -Recurse -File -Filter backtest.py -ErrorAction SilentlyContinue |
           Select-Object -First 1 -ExpandProperty FullName
  if ($found) { $Backtest = $found; Write-Host "Ã°Å¸â€œâ€ž Auto-selected backtest: $Backtest" -ForegroundColor DarkYellow }
}
...
if (-not $Python)   { Write-Host "Ã¢ÂÅ’ Python not found at $PythonRel" -ForegroundColor Yellow; if($PauseOnExit){Read-Host "Press Enter"}; exit 3 }
if (-not $Backtest) { Write-Host "Ã¢ÂÅ’ backtest.py not found (tried $BacktestRel + auto-scan)" -ForegroundColor Yellow; if($PauseOnExit){Read-Host "Press Enter"}; exit 4 }
...
if (-not (Get-Process tws -ErrorAction SilentlyContinue)) {
  if ($twspath) { Write-Host "Ã¢â€“Â¶ Starting TWS Paper: $twspath"; Start-Process $twspath; Start-Sleep 8 }
  else { Write-Host "Ã¢ÂÅ’ Could not find tws.exe under C:\Jts\TWS" -ForegroundColor Yellow; if($PauseOnExit){Read-Host "Press Enter"}; exit 5 }
}
...
if (-not $listenOK) {
  Write-Host "Ã¢ÂÅ’ TWS Paper API NOT READY (7497 not listening within $WaitListenSec s)" -ForegroundColor Yellow
  netstat -ano | Select-String ":7497" | Out-Host
  if($PauseOnExit){Read-Host "Press Enter"}; exit 1
}

Write-Host "Ã¢Å“â€ 7497 is LISTENING" -ForegroundColor Green
...
Write-Host ("Ã¢Å“â€¦ Using {0}:7497" -f $chosenHost) -ForegroundColor Green
...
Write-Host "Ã¢â€“Â¶ Running backtest..." -ForegroundColor Cyan
