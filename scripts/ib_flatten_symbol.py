...
def q(ib, c):
    ...
    return (
        ...
        float(t.last) if t.last else None,
    )
...
def clamp(limit, ref, max_ticks, tick, side):
    ...
    if move > max_move:
        return ref + max_move if side == "BUY" else ref - max_move
    return limit
...
def main():
    ...
    try:
        ...
        for p in ib.positions():
            if (
                getattr(p.contract, "symbol", "") == symbol
                and getattr(p.contract, "secType", "") == "STK"
            ):
                pos = int(p.position)
                c = p.contract
                break
        if pos == 0:
            print(f"[FLATTEN] No position in {symbol}.")
            return
        if c is None:
        ...
        if side == "BUY":
            base = ask or last
            assert base is not None, "No ask/last"
            raw = dround(base * (1 + bps), 2)
            limit = dround(clamp(raw, ask, tick_cap, tk, side), 2)
        else:
            base = bid or last
            assert base is not None, "No bid/last"
            raw = dround(base * (1 - bps), 2)
