...
# ---- utils ----
def is_num(x): return isinstance(x,(int,float)) and not math.isnan(x) and x > 0
def round2(x): return float(f"{x:.2f}")
...
SKIP_TICKS = os.getenv("SKIP_TICKS","true").lower() in ("1","true","yes")  # default: fast
...
def try_ticks_fast(ib, contract, max_wait=3.0):
    if SKIP_TICKS: return None, None, None
    for mdt in (1,3,4):  # realtime -> delayed -> delayed-frozen
        try: ib.reqMarketDataType(mdt)
        except Exception: pass
        ticker = ib.reqMktData(contract, "", False, False)
        start = time.time()
        while time.time()-start < max_wait:
            ask, bid, last, mid, close = ticker.ask, ticker.bid, ticker.last, ticker.midpoint(), ticker.close
            px = next((p for p in (ask,bid,last,mid,close) if is_num(p)), None)
            if is_num(px):
                return px, {1:"IB-rt",3:"IB-delayed",4:"IB-delayed-frozen"}[mdt], ticker
            ib.sleep(0.2)
...
def try_polygon(symbol):
    ...
    import requests
    try:
        r = requests.get(f"https://api.polygon.io/v2/last/trade/{symbol}", params={"apiKey":key}, timeout=5)
        if r.ok:
            px = (r.json() or {}).get("results", {}).get("p")
            if is_num(px): return float(px)
    except Exception: pass
    try:
        r = requests.get(f"https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/tickers/{symbol}",
                         params={"apiKey":key}, timeout=5)
        if r.ok:
            jd = r.json() or {}
            q  = (jd.get("ticker") or {}).get("lastQuote") or {}
            ask = q.get("pAsk"); bid = q.get("pBid")
            if is_num(ask): return float(ask)
            if is_num(bid): return float(bid)
    except Exception: pass
    return None

def logs_contain(trade, code):
    try: return any(f"errorCode={code}" in str(le) for le in (trade.log or []))
...
def wait_status(trade, ib, timeout=4.0):
    ...
    while time.time() < end:
        ...
        if st in ("Submitted","PreSubmitted","PendingSubmit","Cancelled","Filled","ApiCancelled"):
...
if not cands:
    print("GATE: no ALLOW in watch set -> nothing to do."); raise SystemExit(0)
...
if port is None and LIVE_FLAG and FALLBACK:
    print("LIVE refused; trying PAPER fallback"); port = try_connect(ib, live=False)
if port is None: raise RuntimeError("IB API not reachable (LIVE/PAPER).")
...
if not is_num(px):
    ...
    if not is_num(px): raise RuntimeError("No price via ticks/hist/polygon.")
    src = "IB-hist/Polygon"; mode = "MKT"

try:
    if 'ticker' in locals() and ticker: ib.cancelMktData(contract)
except Exception: pass

ref = float(px) + float(ASK_ADD)
lmt = round2(ref); tp = round2(lmt*(1+float(TP_PCT))); sl = round2(lmt*(1-float(SL_PCT)))

# ---- place parent then children ----
parent = (LimitOrder("BUY", QTY, lmt, tif=TIF) if mode=="LMT" else MarketOrder("BUY", QTY, tif=TIF)); parent.transmit=False
...
if mode=="LMT" and pstat=="Cancelled" and logs_contain(tradeParent, 163):
    parent2 = MarketOrder("BUY", QTY, tif=TIF); parent2.transmit=False
    tradeParent = ib.placeOrder(contract, parent2); pstat = wait_status(tradeParent, ib, timeout=4.0); mode="MKT"
if pstat not in ("Submitted","PreSubmitted","PendingSubmit","Filled"):
    for le in (tradeParent.log or []): print("LOG:", le)
    raise RuntimeError(f"Parent not accepted (status={pstat}).")

poid = tradeParent.order.orderId
tpChild = LimitOrder("SELL", QTY, tp, tif=TIF); tpChild.parentId=poid; tpChild.transmit=False
slChild = StopOrder("SELL", QTY, sl, tif=TIF);  slChild.parentId=poid; slChild.transmit=True
ib.placeOrder(contract, tpChild); ib.placeOrder(contract, slChild)

print(f"SESSION: {'LIVE' if live_now else 'PAPER'} on port {port}")
print(f"PRICE_SRC: {src}  MODE: {mode}")
print(f"PLACED: {symbol} x{QTY} @ {('MKT' if mode=='MKT' else lmt)} | TP {tp} | SL {sl} | parentId={poid}")
...
with open(logf,"a",newline="",encoding="utf-8") as f:
    w=csv.writer(f)
    if not exists: w.writerow(["ts","session","symbol","score","mode","qty","lmt","tp","sl","parentId","title","url"])
    w.writerow([datetime.now().isoformat(timespec="seconds"), "LIVE" if live_now else "PAPER",
                symbol, f"{score:.4f}", mode, QTY, (lmt if mode!="MKT" else "MKT"), tp, sl, poid, title, url])
ib.disconnect()
