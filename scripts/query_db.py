...
from hybrid_ai_trading.data.store.database import SessionLocal, Price, News, Base, engine
...
def query_news_with_prices(limit=None, symbol="AAPL", export_csv=True, export_db=True):
    ...
    for n in headlines:
        ...
        same_day_return = (
            ...
            if price_today and price_today.open else None
        )

        next_day_return = (
            (price_next.close - price_next.open) / price_next.open * 100
            if price_next and price_next.open else None
        )
