...
def connect():
    ...
    try:
        ...
        btnDisconnect.configure(state="normal")
...
def disconnect():
    try:
        ib.disconnect()
        btnConnect.configure(state="normal")
        btnDisconnect.configure(state="disabled")
...
def send(side):
    sym = symVar.get().strip().upper() or "AAPL"
    try:
        qty = int(qtyVar.get() or "1")
    except:
        qty = 1
    try:
        bps = int(bpsVar.get() or "5")
    except:
        bps = 5
    ...
    log(
        f"[PLAN] {side} {qty} {sym} @ ~{limit} (TIF={tif}) notionalâ‰ˆ${notional:,.2f} [eff_bps={eff_bps}]"
    )
...
util.startLoop()
root = tk.Tk()
root.title("GUI Trading Console (Paper IOC)")
...
# Buttons
style.configure(
    "TButton", background=BTN, foreground=FG, borderwidth=0, focusthickness=0, padding=6
)
...
ttk.Checkbutton(
    ...
    text="Stay on top",
    variable=topVar,
    command=lambda: root.attributes("-topmost", topVar.get()),
).grid(row=row, column=6, padx=6, sticky="w")

row += 1
btnConnect = ttk.Button(frm, text="Connect", command=connect)
...
out.grid(row=row, column=0, columnspan=7, sticky="nsew")
