...
if final_state is None:
    ...
    for entry in list(trade.log):
        ev = getattr(entry, "event", None)
        st = getattr(ev, "orderState", None)
        if st and not any(is_unknown(getattr(st, fld, None)) for fld in
                          ("initMarginChange","maintMarginChange","equityWithLoanChange","commission")):
            final_state = st
            break
...
if final_state:
    ...
    print("No final what-if yet.",
          "Saw placeholder first." if saw_placeholder else "No placeholder seen. Will keep alive longer next run.")
