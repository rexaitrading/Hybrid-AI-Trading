# Kraken ETH1H wrapper (ASCII-only, scheduler-proof)
...
$cli = Join-Path $root "scripts\run_eth1h_kraken_paper.py"
$lgd = Join-Path $root "logs"
$log = Join-Path $lgd  "eth1h_kraken_runner_ps1.log"
...
Write-Log $log "[$stamp] Running ETH1H Kraken Runner..."
Push-Location $root
try{
  & $py $cli 2>&1 | ForEach-Object { Write-Log $log $_ }
  Write-Log $log "[Kraken done] ExitCode=0"
