...
def main():
    ...
    abort_no_quote = os.getenv("ABORT_IF_NO_QUOTE", "true").lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    print(
        f"[CHECK] SYMBOL={symbol}  SIDE={side}  QTY={qty}  BPS={bps}  TIF={tif}  TICK_CAP={tick_cap}"
    )
    ib = IB()
    ...
    try:
        ...
        if abort_no_quote and not (valid(bid) or valid(ask)):
            print("[ABORT] No live bid/ask (likely after-hours).")
            return

        base = (
            (ask if side == "BUY" else bid)
            if (valid(ask) if side == "BUY" else valid(bid))
            else last
        )
        if not valid(base):
        ...
        ref = (
            ...
            else (bid if (side == "SELL" and valid(bid)) else base)
        )
        limit = clamp_by_ticks(raw, ref, tick_cap, tk, side)
        notional = d2(limit * qty)

        print(
            f"[PREVIEW] Will {side} {qty} {symbol}  @ ~{limit}  (TIF={tif}, outsideRth={outside_rth})  notionalâ‰ˆ${notional:,.2f}"
        )
