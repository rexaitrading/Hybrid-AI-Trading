...
def main():
    ...
    by_symbol = defaultdict(lambda: {"orders":0,"filled":0,"cancelled":0,"abort":0,"submitted":0,
    ...
    for r in rows:
        ...
        if status == "FILLED":
            ...
            if side == "BUY":
                S["buy_qty"] += qty
                if avg: S["buy_notional"] += qty * avg
            elif side == "SELL":
                S["sell_qty"] += qty
                if avg: S["sell_notional"] += qty * avg
    ...
    for sym, S in by_symbol.items():
        ...
        if S["abort"] >= 2:
            suggestions.append(f"- **{sym}**: Many ABORTs → after-hours. Keep `ABORT_IF_NO_QUOTE=true` (good).")

        # if low fill rate → suggest +1 bps for tomorrow
        if fillable >= 3 and fill_rate < 60:
            bump = 1 if sym in ("MSFT","NVDA") else 1
            suggestions.append(f"- **{sym}**: Fill rate {fill_rate:.0f}% (<60%). Try **+{bump} bps** slippage tomorrow.")

        # if 100% IOC cancels during RTH (no ABORTs) → bump bps too
        if S["cancelled"] >= 3 and S["abort"] == 0 and S["filled"] == 0:
            suggestions.append(f"- **{sym}**: All IOC cancelled. Increase slippage bps slightly or wait for liquidity windows.")
    ...
    md.append("")
    md.append("## Summary")
    md.append(f"- Total orders: **{total_orders}**  |  Filled: **{by_status['FILLED']}**  |  Cancelled: **{by_status['CANCELLED']}**  |  Submitted(resting): **{by_status['SUBMITTED']}**  |  Abort: **{by_status['ABORT']}**")
    md.append("")
    md.append("## Per-Symbol")
    for sym, S in sorted(by_symbol.items()):
        fillable = S["filled"] + S["cancelled"]
        fill_rate = (S["filled"]/fillable)*100 if fillable>0 else 0.0
        md.append(f"**{sym}** — orders {S['orders']}, filled {S['filled']}, cancelled {S['cancelled']}, abort {S['abort']}, fill-rate {fill_rate:.0f}%  |  realized PnL: **${S['realized_pnl']:.2f}**")
    md.append("")
    ...
    md.append("")
    md.append("> Note: PnL is a simple realized estimate using order avg fills. Open PnL not included.")
