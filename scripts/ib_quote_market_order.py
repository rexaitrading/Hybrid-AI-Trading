...
def last_loss_within_minutes(minutes: int) -> bool:
    ...
    for r in rows:
        ...
        side = r["side"].upper()
        q = float(r["qty"] or 0)
        px = float(r["avgFill"] or 0 or 0.0)
        bysym.setdefault(s, {"b": [], "s": []})
        (bysym[s]["b"] if side == "BUY" else bysym[s]["s"]).append(
            (datetime.fromisoformat(r["ts"]), q, px)
        )
...
def guard_and_plan(ib, *, symbol, side, qty, tif, max_notional, tick_cap, preset_bps):
    ...
    if side == "BUY":
        ...
        if not valid(base):
            return False, "no_ask_or_last", None
        raw = d2(base * (1 + bps))
        ...
        if not valid(base):
            return False, "no_bid_or_last", None
        raw = d2(base * (1 - bps))
...
def main():
    ...
    try:
        ...
        print(
            f"[PLAN] {side} {qty} {symbol} @ ~{limit} (TIF={tif}) notionalâ‰ˆ${notional:,.2f}  [eff_bps={eff_bps}]"
        )
        tr = ib.placeOrder(
