Write-Host '=== Session Preflight (IB Gateway / Paper) — probe only ==='
...
if ($tnc.TcpTestSucceeded) { Write-Host '✅ Port reachable' } else { Write-Host '❌ Port NOT reachable' }
...
if ($cleared) { Write-Host '✅ sockets clear' } else { Write-Host '⚠️ sockets not fully clear; proceeding' }
