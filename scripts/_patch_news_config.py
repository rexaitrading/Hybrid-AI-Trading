...
np["aggregate"] = True
np["benzinga"] = {"enabled": True, "weight": float(np.get("benzinga",{}).get("weight", 1.0))}
np["polygon"]  = {"enabled": True, "weight": float(np.get("polygon",{}).get("weight", 1.0))}
rss = np.get("rss") or {}
rss["enabled"] = True
rss["per_feed_max"] = int(rss.get("per_feed_max", 8))
rss["google_template"] = rss.get("google_template") or "https://news.google.com/rss/search?q={sym}+stock&hl=en-US&gl=US&ceid=US:en"
rss["yahoo_template"]  = rss.get("yahoo_template")  or "https://feeds.finance.yahoo.com/rss/2.0/headline?s={sym}&lang=en-US"
...
print(f"UPDATED: sweep_hours_back={hours_back}, sweep_limit={limit}, providers: benzinga/polygon/rss enabled")