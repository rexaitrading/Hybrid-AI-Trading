...
def good(state):
    return state and not is_unknown(getattr(state,"initMarginChange",None)) \
...
best = None
for attempt in range(8):                      # ~8 attempts over ~16–20s
...
if best:
    ...
    print("commission:", getattr(best,"commission",None), getattr(best,"commissionCurrency",""))
