...
def now_local():
    # Use local machine time (you’re in Vancouver)
    return datetime.now()
    ...
    with p.open("r", encoding="utf-8", errors="ignore") as f:
        ...
        for r in rd:
            if (r.get("ts","").split("T")[0] != today) or (r.get("status","").upper()!="FILLED"): 
                continue
            side=r.get("side","").upper(); qty=float(r.get("qty") or 0.0)
            avg = r.get("avgFill"); avg = float(avg) if avg not in ("",None) else None
            sym = (r.get("symbol") or "").upper()
            if avg is None or qty<=0: continue
            if side=="BUY":  buys.append((sym, qty, avg))
            if side=="SELL": sells.append((sym, qty, avg))
...
def guard(ib, *, symbol, side, qty, tif, max_notional, tick_cap, slip_bps):
    ...
    return True, {"contract":c, "bid":bid, "ask":ask, "last":last, "tk":tk, "limit":limit, "notional":notional}
...
def main():
    ...
    try:
        ok, info = guard(ib, symbol=symbol, side=side, qty=qty, tif=tif,
                         max_notional=max_notional, tick_cap=tick_cap, slip_bps=slip)
        if not ok:
            print(f"[ABORT] Guard blocked: {info}")
            _log(symbol, side, qty, None, None, None, None, "ABORT", 0, "", info)
            return

        c  = info["contract"]; bid,ask,last = info["bid"],info["ask"],info["last"]
        limit = info["limit"]; notional = info["notional"]
        ...
        _log(symbol, side, qty, bid, ask, last, limit, tr.orderStatus.status, tr.orderStatus.filled, tr.orderStatus.avgFillPrice, "")
    finally:
        if keep_open:
...
def _log(symbol, side, qty, bid, ask, last, limit, status, filled, avgFill, note):
    ...
    with f.open("a", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        if new:
            w.writerow(["ts","symbol","side","qty","bid","ask","last","limit","status","filled","avgFill","note"])
