...
def main():
    host=os.getenv("IB_HOST","127.0.0.1"); port=int(os.getenv("IB_PORT","7498")); cid=int(os.getenv("IB_CLIENT_ID","301"))
    sym=os.getenv("SYMBOL","AAPL").upper(); side=os.getenv("SIDE","BUY").upper()
    qty=int(os.getenv("QTY","1")); bps=int(os.getenv("SLIPPAGE_BPS","5")); tick_cap=int(os.getenv("TICK_CAP","20") or 20)
    ...
    try:
        ...
        if not (v(bid) or v(ask) or v(last)):
            try: ib.reqMarketDataType(3)   # DELAYED
            except: pass
            t=ib.reqMktData(c,"",False,False); ib.sleep(1.0)
            bid = float(t.bid) if v(t.bid) else None
            ask = float(t.ask) if v(t.ask) else None
        ...
        if side=="BUY":
            ...
            if not v(base): print("[ABORT] no price even after delayed fallback"); return
        ...
        if notional<=0: print("[ABORT] invalid notional"); return

        o = LimitOrder(side, qty, limit, tif=os.getenv("TIF","IOC"), outsideRth=os.getenv("OUTSIDE_RTH","true").lower() in ("1","true","yes"))
        tr=ib.placeOrder(c,o)
        print(f"[PLAN] {side} {qty} {sym} @ ~{limit} (IOC) notional≈${notional}")
