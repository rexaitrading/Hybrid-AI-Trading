...
def main():
    ...
    if not is_num(px):
        ...
        if not is_num(px):
            raise RuntimeError(f"No price for {SYMBOL} via ticks, IB-hist, or Polygon. Enable delayed data or set POLYGON_KEY.")
        src = "IB-hist/Polygon"
    ...
    if pstat not in ("Submitted","PreSubmitted","PendingSubmit","Filled"):
        ...
        for le in (tradeParent.log or []):
            print("LOG:", le)
        raise RuntimeError(f"Parent not accepted (status={pstat}). Aborting children to avoid orphans.")
    ...
    print(f"PLACED: {SYMBOL} x{QTY} @ {('MKT' if mode=='MKT' else lmt)} | TP {tp} | SL {sl} | {'LIVE' if LIVE else 'PAPER'} | parentId={poid}")
