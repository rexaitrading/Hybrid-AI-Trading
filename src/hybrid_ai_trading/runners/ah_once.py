...
def compute_marketable_limit(ib: IB, contract: Stock, side: str, offset: float) -> float:
    ...
    ib.sleep(1.5)
    bid = float(tk.bid or 0.0); ask = float(tk.ask or 0.0)
    if side == "BUY":
...
def main():
    args = parse_args()
    ib = IB(); ib.connect(args.host, args.port, clientId=args.client_id, timeout=10)
    try: ib.reqMarketDataType(4)  # Delayed-Frozen for paper fills
    except Exception: pass

    contract = Stock(args.symbol, args.dest, "USD"); ib.qualifyContracts(contract)
