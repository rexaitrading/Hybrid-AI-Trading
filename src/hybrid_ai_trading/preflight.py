...
STATE=os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "..", ".runtime", "guard_state.json"))
...
def get_usdc()->float:
    ...
    if not key or not os.path.exists(key): return -1.0
    if ccxt is None: return -1.0
    try:
        ...
        c=load_client(); b=c.fetch_balance() or {}; free=b.get("free") or {}
...
def main()->None:
    ...
    pairs = env_list("TC_CRYPTO","BTC/USDC,ETH/USDC") if crypto_signal else ["BTC/USDC","ETH/USDC"]
    ...
    # Balances
    usdc = get_usdc()
    if usdc < 0: print("USDC: (unavailable)"); 
    else: print(f"USDC: {usdc:.2f}")
    ...
    print(f"  used today: notional={used_not:.2f} trades={used_tr}  room: notional={room_not} trades={room_tr}")
    ...
    if crypto_signal:
        ...
        for p in pairs:
            sig=crypto_signal(p)
            if not sig or not sig.get("buy"): continue
            eff = min(float(sig["size_quote"]), cap) if cap>0 else float(sig["size_quote"])
            print(f"{p} ({sig['tf']}): entry≈{sig['last']:.2f} size≈{eff:.2f} stop≈{(sig['stop'] or float('nan')):.2f}")
            anyb=True
        if not anyb: print("(no crypto buys now)")
    else:
