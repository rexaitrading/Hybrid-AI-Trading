...
def main() -> None:
    ...
    room_not = "Ã¢Ë†Å¾" if daily_not <= 0 else f"{max(0.0,daily_not - used_not):.2f}"
    room_tr = "Ã¢Ë†Å¾" if daily_tr <= 0 else f"{max(0,daily_tr - used_tr)}"
    print(
    ...
    if crypto_signal:
        ...
        for p in pairs:
            ...
            print(
                f"{p} ({sig['tf']}): entryÃ¢â€°Ë†{sig['last']:.2f} sizeÃ¢â€°Ë†{eff:.2f} stopÃ¢â€°Ë†{(sig['stop'] or float('nan')):.2f}"
            )
            anyb = True
        if not anyb:
    ...
    print("\nCHECK\n-----")
    print("GO" if go else "NO-GO: need Ã¢â€°Â¥ ~6 USDC (or funding to post)")
