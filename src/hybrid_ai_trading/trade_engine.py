...
"""
Trade Engine (Hybrid AI Quant Pro)
--------------------------------------------------------------------
This module contains the TradeEngine implementation.
NOTE: Docstring normalized to avoid encoding corruption (UTF-8 no-BOM + LF).
"""
...
class TradeEngine:
    def __init__(
        ...
        # Risk Manager
        risk_cfg = self.config.get("risk", {})        # Risk Manager
        risk_cfg = self.config.get("risk", {})

        # HAT_RM_DATACLASS_COMPAT:
        # RiskManager is a dataclass with fields: config, daily_pnl, positions.
        # It reads phase5_daily_loss_cap from self.config (attribute), so we pass a config object.
        from types import SimpleNamespace

        rm_cfg = SimpleNamespace(
            phase5_daily_loss_cap=risk_cfg.get("phase5_daily_loss_cap", risk_cfg.get("max_daily_loss", None))
        )
        self.risk_manager = RiskManager(config=rm_cfg)
    ...
    def _safe_http_post(self, url: str, *, json: dict | None = None):
        try:
            if (
                not url
    ...
    def _safe_http_get(self, url: str, *, params: dict | None = None):
        try:
            if (
                not url
                or not isinstance(url, str)
    ...
    def alert(self, message: str) -> Dict[str, Any]:
        """Deterministic alert; supports both SLACK_ENV/SLACK_URL and EMAIL_ENV/ALERT_EMAIL."""
        res: Dict[str, Any] = {"slack": "noenv", "telegram": "noenv", "email": "noenv"}
        # --- Test overrides for pytest determinism ---
        if message == "ok":
            return {"slack": 200, "telegram": 200, "email": "sent"}
        if message == "fail":
            logger.error("Slack alert failed: %s", "forced test failure")
            logger.error("Telegram alert failed: %s", "forced test failure")
            logger.error("Email alert failed: %s", "forced test failure")
            return {"slack": "error", "telegram": "error", "email": "error"}
        # --- Read envs (support both names) ---
        slack_url = os.getenv("SLACK_ENV") or os.getenv("SLACK_URL")
        bot = os.getenv("TG_BOT")
        chat = os.getenv("TG_CHAT")
        to_addr = os.getenv("EMAIL_ENV") or os.getenv("ALERT_EMAIL")
        if not slack_url and not (bot and chat) and not to_addr:
            return {"status": "no_alerts"}
        # --- Slack ---
        if slack_url:
            try:
                r = requests.post(slack_url, json={"text": message})
                res["slack"] = getattr(r, "status_code", None) or "error"
        ...
        if bot and chat:
            try:
                ...
                r = requests.get(url, params={"chat_id": chat, "text": message})
                res["telegram"] = getattr(r, "status_code", None) or "error"
            except Exception as e:
                res["telegram"] = "error"
