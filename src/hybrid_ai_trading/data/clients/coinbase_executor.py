...
import os, uuid, math, argparse, json
...
def market_buy_quote(client: RESTClient, product_id: str, quote_size: float, live: bool) -> dict:
    ...
    if not live:
        return {"dry_run": True, "op": "market_buy_quote", "product_id": product_id, "quote_size": quote_sz}
...
def limit_buy_gtc(client: RESTClient, product_id: str, base_size: float, limit_price: float, live: bool) -> dict:
    product = client.get_product(product_id)
    ...
    if not live:
        return {"dry_run": True, "op": "limit_buy_gtc", "product_id": product_id, "base_size": base_sz, "limit_price": limit_px}
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Coinbase Advanced Trade executor (DRY-RUN by default)")
    ap.add_argument("--pair", default="BTC-USD", help='Product pair like "BTC-USD" or "BTC-USDC"')
    ap.add_argument("--market-buy-quote", type=float, help="Spend this much quote currency (e.g., 5 USD)")
    ap.add_argument("--limit-buy-base", type=float, help="Buy this base amount (e.g., 0.0002 BTC)")
    ap.add_argument("--below-percent", type=float, default=5.0, help="Limit % below current price (used with --limit-buy-base)")
