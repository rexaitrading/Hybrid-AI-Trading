...
STATE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), '..', '..', '..', '.runtime', 'guard_state.json')
...
def check(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if broker == 'kraken' and max_trade_quote > 0 and notional > max_trade_quote:
        return {"ok": False, "error": "cap_violation", "hint": f"Reduce quote to <= {max_trade_quote:.2f}"}
    if broker == 'ibkr' and max_trade_shares > 0 and shares > max_trade_shares:
        return {"ok": False, "error": "cap_violation", "hint": f"Reduce shares to <= {max_trade_shares:.2f}"}

    # daily counts
    if daily_trades > 0 and (day_state["trades"] + 1) > daily_trades:
        return {"ok": False, "error": "daily_trades_limit", "hint": f"Limit {daily_trades} reached today"}

    if broker == 'kraken' and daily_notional > 0 and (day_state["notional"] + notional) > daily_notional:
        return {"ok": False, "error": "daily_notional_limit", "hint": f"Daily notional cap {daily_notional:.2f} reached"}
