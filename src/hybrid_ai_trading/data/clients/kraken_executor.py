...
def main() -> None:
    ...
    last = float(ticker["last"]) if ticker.get("last") else float((ticker.get("info") or {}).get("c", [0])[0])
    ...
    m = ex.market(symbol)
    print({"symbol": symbol, "price": last, "precision": m.get("precision", {}), "limits": m.get("limits", {})})
