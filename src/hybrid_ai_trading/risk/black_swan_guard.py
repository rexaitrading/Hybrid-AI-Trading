"""
Black Swan Guard (Hybrid AI Quant Pro v12.7 Ã¢â‚¬â€œ Hedge Fund OE Grade, 100% Coverage)
---------------------------------------------------------------------------------
...
class BlackSwanGuard:
    ...
    def __init__(self) -> None:
        # Map: source Ã¢â€ â€™ reason
        self.events: Dict[str, str] = {}
        logger.info("Ã¢Å“â€¦ BlackSwanGuard initialized | events=%s", self.events)
    ...
    def trigger_event(self, source: str, reason: str = "unspecified") -> None:
        ...
        logger.warning(
            "[BlackSwanGuard] Ã¢Å¡Â Ã¯Â¸Â Triggered | source=%s, reason=%s",
            source,
            reason,
        )
    ...
    def clear_event(self, source: str) -> None:
        ...
        if source in self.events:
            logger.info("[BlackSwanGuard] Ã¢Å“â€¦ Event Cleared | source=%s", source)
            self.events.pop(source)

    def clear_all(self) -> None:
        """Clear all active events."""
        if self.events:
            logger.info("[BlackSwanGuard] Ã¢Å“â€¦ All events cleared")
            self.events.clear()
    ...
    def filter_signal(self, signal: str) -> str:
        ...
        - If active and signal is BUY/SELL Ã¢â€ â€™ returns "HOLD"
        - HOLD always passes
        ...
        if signal.upper() in {"BUY", "SELL"}:
            logger.warning(
                "[BlackSwanGuard] Ã¢ÂÅ’ Trade Blocked | signal=%s, active_events=%s",
                signal,
