"""
GateScore (Hybrid AI Quant Pro v37.5 – Hedge Fund Grade, 100% Coverage)
=======================================================================
...
- Adaptive threshold based on detected regime (if enabled):
  * bull   → threshold lower (easier to allow trades).
  * bear   → threshold higher (harder to allow trades).
  * crisis → threshold much higher (strict risk mode).
  * neutral/sideways → base threshold.
- Guards:
  * Disabled gate → always allow (audit-friendly path).
  * Missing models → ignored or veto depending on strict_missing.
  * Invalid/exception in score → treated as 0.
  * total weight <= 0 → block trade.
"""
...
class GateScore:
    ...
    def _normalize_weights(self, weights: Dict[str, float]) -> Dict[str, float]:
        ...
        if total <= 0:
            logger.warning("⚠️ Invalid weights (sum<=0), assigning equal weights")
            n = len(weights)
    ...
    def allow_trade(
        ...
        for m in self.models:
            ...
            if m not in inputs:
                if self.strict_missing:
                    ...
                    logger.info("[GateScore] ⚠️ Missing model %s → ignored", m)
                    continue
        ...
        if not contributing:
            logger.warning("[GateScore] ❌ No contributing models → block trade")
            if self.audit_mode:
                return False, 0.0, self.base_threshold, regime
            return False

        if total_weight <= 0:
            logger.warning("[GateScore] ❌ Total weight=0 → block trade")
            if self.audit_mode:
                return False, 0.0, self.base_threshold, regime
            return False
    ...
    def _safe_score(self, m: str, val) -> float:
        ...
        try:
            ...
            logger.warning("⚠️ GateScore _safe_score failed for %s: %s", m, e)
            return 0.0
