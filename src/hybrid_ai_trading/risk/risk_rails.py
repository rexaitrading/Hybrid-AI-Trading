...
def max_order_size(
    ...
    if qty > cap:
        return RiskDecision(
            "block", f"max_order_size_exceeded cap={cap}", {"qty": qty, "asset": asset}
        )
    return RiskDecision("ok")
...
def daily_pnl_cap(current_pnl: float, cap_abs: float | None) -> RiskDecision:
    ...
    if current_pnl <= -abs(cap_abs):
        return RiskDecision(
            "block", f"daily_loss_cap {current_pnl} <= -{cap_abs}", {"pnl": current_pnl}
        )
    return RiskDecision("ok")
...
def drawdown_cap(
    ...
    if dd > dd_max:
        return RiskDecision(
            "block",
            f"drawdown_breach dd={dd:.4f} > {dd_max}",
            {"equity": equity, "peak": peak_equity},
        )
    return RiskDecision("ok")
