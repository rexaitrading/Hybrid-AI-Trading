"""
Market Logger (Hybrid AI Quant Pro v8.2 �?,???o OE Hedge-Fund Grade, Polished)
------------------------------------------------------------------------
Responsibilities:
...
class MarketLogger:
    ...
    def start_logging(self) -> None:
        ...
        logger.info("�Ys?,? Market logging started for: %s", ", ".join(self.symbols))
        try:
            self.ib.run()
        except KeyboardInterrupt:
            logger.info("�Y?????~ Interrupted by user. Shutting down...")
            self.shutdown()
        except Exception as exc:  # noqa: BLE001
        ...
        if self.ib:
            for sym, ticker in list(self.subscriptions.items()):
                try:
                    ...
                    logger.warning("�s�️ Failed to unsubscribe %s: %s", sym, exc)
            try:
                self.ib.disconnect()
                logger.info("�Y????' Disconnected from IBKR")
            except Exception as exc:  # noqa: BLE001
                logger.warning("�s�️ IBKR disconnect issue: %s", exc)
            finally:
                self.ib = None
