...
from ib_insync import (
    IB, Stock, Contract, Trade, OrderStatus,
    LimitOrder, StopOrder, StopLimitOrder, TagValue
)
...
def spread_guard_ok(q: Quotes, cap_bps: float) -> Tuple[bool, Optional[float]]:
    b, a = q.bid, q.ask
    if not (b and a and b > 0 and a >= b): return False, None
    mid = (b + a) / 2.0
...
# ---------- dedupe / what-if ----------
def dedupe_open_orders(ib: IB, symbol: str, side: str, mode: str="cancel_older") -> Tuple[list[Trade], list[Trade]]:
    side = side.upper()
...
def run(
    ...
    symbol: str, side: str,
    qty: Optional[int], risk_cash: Optional[float],
    tif: str, outside_rth: bool, adaptive: Optional[str],
    orderref_prefix: str, slip_pct: float, ticks_clamp: int, fallback_ticks: int,
    spread_bps_cap: float, early_open_block_min: int,
    dedupe_mode: str, whatif: bool, autoreprice_sec: int, cooldown_min: int,
    tp_pct: float, sl_pct: float, slack_webhook: Optional[str]
) -> None:
    if PAUSE_FLAG.exists():
        print("PAUSE present; abort."); return
    ...
    if COOLDOWNS.exists():
        try: cooldowns = json.loads(COOLDOWNS.read_text("utf-8") or "{}")
    ...
    for t in cancelled:
        _jsonl({"event":"dedupe_cancel","symbol":t.contract.symbol,"action":t.order.action,
    ...
    # Sizing
    if qty is None:
        if not risk_cash: raise RuntimeError("--qty not given and --risk-cash missing")
    ...
    tr_parent, tr_take, tr_stop = place_bracket(
        ib, contract, side, qty, lmt, tp_pct, sl_pct, tif, outside_rth, adaptive, order_ref
    )
    placed = {"event":"placed","symbol":symbol,"side":side,"qty":qty,"lmt":lmt,
              "tp_pct":tp_pct,"sl_pct":sl_pct,"tif":tif,"outsideRth":outside_rth,
              "adaptive":adaptive,"orderRef":order_ref,
              "permId":int(tr_parent.orderStatus.permId or tr_parent.order.orderId or 0),
              "spread_bps":spr_bps,
              "quotes":{"bid":q.bid,"ask":q.ask,"last":q.last,"close":q.close,"minTick":q.minTick}}
    print("Placed:", placed); _jsonl(placed)

    # Auto-reprice once if stuck
    if autoreprice_sec > 0:
        deadline = time.time() + autoreprice_sec
        while time.time() < deadline and tr_parent.orderStatus.status in ("PreSubmitted","Submitted"):
        ...
        if tr_parent.orderStatus.status in ("PreSubmitted","Submitted"):
            ...
            tr_parent2, tr_take2, tr_stop2 = place_bracket(
                ib, contract, side, qty, lmt2, tp_pct, sl_pct, tif, outside_rth, adaptive, order_ref + "_R1"
            )
            placed2 = {**placed, "event":"replaced","lmt":lmt2,
                       "permId":int(tr_parent2.orderStatus.permId or tr_parent2.order.orderId or 0)}
            print("Replaced:", placed2); _jsonl(placed2)
    ...
    # Snapshot
    oo = [{"permId": int(t.orderStatus.permId or t.order.orderId),
           ...
           "tif": t.order.tif, "outsideRth": getattr(t.order, "outsideRth", None),
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    run(
        host=a.host, port=a.port, client_id=a.client_id,
        symbol=a.symbol, side=a.side,
        qty=a.qty, risk_cash=a.risk_cash,
        tif=a.tif, outside_rth=bool(a.outside_rth), adaptive=a.adaptive,
        orderref_prefix=a.orderref_prefix, slip_pct=float(a.slip_pct),
