...
"""
IBClient (Hybrid AI Quant Pro �?,???o minimal, safe wrapper)
...
class IBConfig:
    ...
    port: int = int(os.getenv("IB_GATEWAY_PORT", "4002"))  # TWS paper by default
...
def get_last_prices(symbols, client_id=3021):
    """
    Return dict {SYM: float} using IB snapshot market data.
    Fail-closed if any symbol is missing/<=0.
    """
    if not symbols:
        raise RuntimeError("get_last_prices: empty symbols")

    cfg = IBConfig(client_id=int(client_id))
    out: Dict[str, float] = {}

    with IBClient(cfg).session() as c:
        try:
            c.ib.reqMarketDataType(1)
        except Exception:
            pass

        for sym in symbols:
            s = str(sym).upper()
            t = c.ib.reqMktData(Stock(s, "SMART", "USD"), "", snapshot=True)
            c.ib.sleep(1.5)
            ...
            if px is None:
                raise RuntimeError(f"ib_snapshot_missing_or_bad_price:{s}")
