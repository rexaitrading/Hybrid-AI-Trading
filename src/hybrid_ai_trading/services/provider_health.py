...
def health_providers(providers: List[str] = None) -> Dict[str,Any]:
    ...
    for prov, sym, key in checks:
        ...
        try:
            ...
            out.append({"provider": prov, "symbol": sym, "lat_ms": round(lat,2), "ok": ok, "resp": r})
        except Exception as e:
            lat = (perf_counter()-t0)*1000.0
            out.append({"provider": prov, "symbol": sym, "lat_ms": round(lat,2), "ok": False, "error": f"{type(e).__name__}: {e}"})
    return {"checks": out}