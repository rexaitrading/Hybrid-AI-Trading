...
from ib_insync import Stock, Forex, LimitOrder
...
def acct_bp_usd(ib):
    ...
    for r in ib.accountValues():
        if r.tag == "AvailableFunds" and r.currency == "CAD":
            try: cad=float(r.value)
            except: pass
        if r.tag == "AvailableFunds" and r.currency == "USD":
            try: usd=float(r.value)
            except: pass
    fx = Forex("USDCAD"); ib.reqMktData(fx, "", False, False); ib.sleep(1.2)
    tick = ib.ticker(fx); rate = tick.last or tick.marketPrice() or tick.close or 1.35
...
def _cancel_if_active(ib, trade_or_order):
    ...
    try:
        ...
        if st not in ("Cancelled", "Filled", "ApiCancelled"):
            ib.cancelOrder(trade_or_order if hasattr(trade_or_order, "orderType") else trade_or_order.order)
    except Exception:
        pass
...
def sanity_probe(symbol="AAPL", qty=1, cushion=0.10, allow_ext=True, force_when_closed=False):
    ...
    with ib_session() as ib:
        ...
        acct = require_paper(ib); out["account"]=acct
        now_et = _now_et()
        ok_time, sess = in_trading_window(now_et, allow_ext)
        out["session"]= {"now_et": now_et.isoformat() if hasattr(now_et,'isoformat') else str(now_et),
                         "session": sess, "allow_ext": bool(allow_ext), "ok_time": bool(ok_time)}
        ...
        bp, meta = acct_bp_usd(ib); out["funds"]= {**meta, "bp_usd_est": round(bp,2)}
        c = Stock(symbol, "SMART", "USD")
        ib.reqMktData(c, "", False, False); ib.sleep(1.2)
        t = ib.ticker(c); px = t.last or t.marketPrice() or t.close or 0.0
        if not px: raise RuntimeError(f"No price for {symbol}")
        out["px"]= round(px,4)
        ...
        out["order"]={"action":"BUY","qty":qty,"limit":safe_px,"status":st,
