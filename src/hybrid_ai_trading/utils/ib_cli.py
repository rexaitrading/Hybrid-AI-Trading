...
from .structured_log import setup_logging, get_logger
from .ib_conn import connect_ib, ib_session, DEFAULT_HOST, DEFAULT_PORT, DEFAULT_CLIENT_ID
...
def _add_common(p):
    ...
    p.add_argument("--json", action="store_true", help="Emit structured JSON logs (default on when --log-file given)")
...
def cmd_ping(args):
    ...
    with ib_session(host=args.host, port=args.port, client_id=args.client_id, timeout=args.timeout, market_data_type=args.mdt, log=args.log) as ib:
...
def cmd_open_orders(args):
    _setup_cli_logging(args)
    with ib_session(host=args.host, port=args.port, client_id=args.client_id, timeout=args.timeout, market_data_type=args.mdt, log=args.log) as ib:
        ops = [(o.order.permId, o.order.action, o.order.totalQuantity, o.orderState.status) for o in ib.openOrders()]
...
def cmd_health(args):
    ...
    with ib_session(host=args.host, port=args.port, client_id=args.client_id, timeout=args.timeout, market_data_type=args.mdt, log=args.log) as ib:
        ...
        print("serverTime:", ib.reqCurrentTime())
        print("accounts:", ib.managedAccounts())
        c = _stock("AAPL","SMART","USD"); ib.qualifyContracts(c)
        t = ib.reqMktData(c, "", False, False); ib.sleep(2.0)
...
def main(argv: Optional[list[str]] = None):
    ...
    _add_common(p_ping); p_ping.set_defaults(func=cmd_ping)

    p_quote = sub.add_parser("quote", help="Get a quick quote (bid/ask/last)")
    p_quote.add_argument("symbol"); p_quote.add_argument("--exchange", default="SMART"); p_quote.add_argument("--currency", default="USD"); p_quote.add_argument("--wait", type=float, default=2.0)
    _add_common(p_quote); p_quote.set_defaults(func=cmd_quote)

    p_wi = sub.add_parser("whatif", help="Simulate a limit order (no transmit)")
    p_wi.add_argument("symbol"); p_wi.add_argument("--side", choices=["BUY","SELL"], required=True)
    p_wi.add_argument("--qty", type=float, required=True); p_wi.add_argument("--limit", type=float, required=True)
    p_wi.add_argument("--exchange", default="SMART"); p_wi.add_argument("--currency", default="USD")
    _add_common(p_wi); p_wi.set_defaults(func=cmd_whatif)

    p_pos = sub.add_parser("positions", help="List positions")
    _add_common(p_pos); p_pos.set_defaults(func=cmd_positions)

    p_ops = sub.add_parser("open-orders", help="List open orders")
    _add_common(p_ops); p_ops.set_defaults(func=cmd_open_orders)

    p_health = sub.add_parser("health", help="End-to-end health: ping + AAPL delayed quote + what-if")
    _add_common(p_health); p_health.set_defaults(func=cmd_health)
