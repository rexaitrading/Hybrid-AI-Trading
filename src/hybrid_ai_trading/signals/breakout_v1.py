"""
BreakoutV1Signal (Hybrid AI Quant Pro v24.0 Ã¢â‚¬â€œ Hedge-Fund Grade, Wrapper-Aligned)
-------------------------------------------------------------------------------
Logic:
- BUY if last close > max(highs of previous N bars)
- SELL if last close < min(lows of previous N bars)
- HOLD otherwise
- Tie case: last_close == high == low Ã¢â€ â€™ SELL (risk-off bias)
...
class BreakoutV1Signal:
    ...
    def generate(
        ...
        try:
            ...
            logger.error("Ã¢ÂÅ’ Failed to fetch bars for %s: %s", symbol, e)
        ...
        try:
            ...
            logger.error("Ã¢ÂÅ’ Failed to parse bars for %s: %s", symbol, e)
            self._log_decision(symbol, "HOLD", "invalid_data")
            return "HOLD" if not audit else ("HOLD", 0.0, 0.0, 0.0)

        if any(math.isnan(x) for x in closes + highs + lows):
            logger.error("Ã¢ÂÅ’ NaN detected in bars for %s", symbol)
