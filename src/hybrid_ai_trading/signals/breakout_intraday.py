"""
Breakout Intraday Signal (Hybrid AI Quant Pro v24.1 – Hedge-Fund Grade, Polished)
---------------------------------------------------------------------------------
Logic:
...
class BreakoutIntradaySignal:
    ...
    def generate(self, symbol: str, bars: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if self.lookback <= 0:
            logger.info("Invalid window (<=0) → HOLD")
        ...
        if any(math.isnan(x) for x in closes + highs + lows):
            logger.warning("NaN detected → HOLD")
        ...
        if high == low == last_close:
            logger.info("Tie case (flat range) → SELL")
            return {"symbol": symbol, "signal": "SELL", "reason": "tie_case"}

        logger.debug("Inside range → HOLD")
