"""
Breakout Intraday Signal (Hybrid AI Quant Pro v24.1 �?,???o Hedge-Fund Grade, Polished)
---------------------------------------------------------------------------------
Logic:
...
class BreakoutIntradaySignal:
    ...
    def generate(self, symbol: str, bars: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if self.lookback <= 0:
            logger.info("Invalid window (<=0) �?????T HOLD")
            return {"symbol": symbol, "signal": "HOLD", "reason": "invalid_window"}

        if len(bars) < self.lookback:
            logger.info("Not enough bars �?????T HOLD")
        ...
        if high == low == last_close:
            logger.info("Tie case (flat range) �?????T SELL")
            return {"symbol": symbol, "signal": "SELL", "reason": "tie_case"}

        logger.debug("Inside range �?????T HOLD")
