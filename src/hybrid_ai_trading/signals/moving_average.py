"""
MovingAverageSignal (Hybrid AI Quant Pro v2.2 Ã¢â‚¬â€œ Hedge-Fund Grade)
-----------------------------------------------------------------
Responsibilities:
...
class MovingAverageSignal:
    ...
    def generate(
        ...
        if not bars:
            logger.info("No bars provided Ã¢â€ â€™ HOLD (not enough bars)")
            return {"signal": "HOLD", "reason": "not enough bars"}

        if len(bars) < self.long_window + 1:
            logger.info("Not enough bars (%s) Ã¢â€ â€™ HOLD", len(bars))
            return {"signal": "HOLD", "reason": "not enough bars"}

        try:
            closes = pd.Series([float(b["c"]) for b in bars if "c" in b])
        except Exception as e:  # noqa: BLE001
            logger.error("Ã¢ÂÅ’ Failed to parse closes for MA signal: %s", e)
            return {"signal": "HOLD", "reason": "failed parse"}

        if closes.empty:
            logger.warning("Missing 'c' field in bars Ã¢â€ â€™ HOLD")
        ...
        if prev_short <= prev_long and curr_short > curr_long:
            ...
            logger.info("MAs equal Ã¢â€ â€™ HOLD")
            sig = "HOLD"
        else:
            logger.info("No crossover Ã¢â€ â€™ HOLD")
            sig = "HOLD"
