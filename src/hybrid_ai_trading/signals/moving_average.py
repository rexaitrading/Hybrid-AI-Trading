"""
MovingAverageSignal (Hybrid AI Quant Pro v2.2 �?,???o Hedge-Fund Grade)
-----------------------------------------------------------------
Responsibilities:
...
class MovingAverageSignal:
    ...
    def generate(
        ...
        if not bars:
            logger.info("No bars provided �?????T HOLD (not enough bars)")
            return {"signal": "HOLD", "reason": "not enough bars"}

        if len(bars) < self.long_window + 1:
            logger.info("Not enough bars (%s) �?????T HOLD", len(bars))
            return {"signal": "HOLD", "reason": "not enough bars"}
        ...
        if closes.empty:
            logger.warning("Missing 'c' field in bars �?????T HOLD")
        ...
        if pd.isna(short_ma.iloc[-1]) or pd.isna(long_ma.iloc[-1]):
            logger.warning("NaN SMA detected �?????T HOLD")
        ...
        if prev_short <= prev_long and curr_short > curr_long:
            ...
            logger.info("MAs equal �?????T HOLD")
            sig = "HOLD"
        else:
            logger.info("No crossover �?????T HOLD")
            sig = "HOLD"
