"""
MACDSignal (Hybrid AI Quant Pro v2.3 �?,???o Hedge-Fund Grade, Test-Aligned)
----------------------------------------------------------------------
Responsibilities:
- Compute MACD (EMA12 �?,???o EMA26) and Signal line (EMA9 of MACD)
- Detect crossovers:
...
class MACDSignal:
    ...
    def generate(
        ...
        if not bars:
            logger.info("No bars provided �?????T HOLD")
        ...
        if closes.empty:
            logger.error("Invalid: no 'c' fields found in bars �?????T HOLD")
            return {"signal": "HOLD", "reason": "invalid"}

        if len(closes) < self.slow + self.signal_window:
            logger.info("Not enough bars (%s) �?????T HOLD", len(closes))
            return {"signal": "HOLD", "reason": "not enough bars"}

        if closes.isna().any():
            logger.warning("NaN detected in closes �?????T HOLD")
        ...
        if math.isnan(macd.iloc[-1]) or math.isnan(signal_line.iloc[-1]):
            logger.warning("NaN MACD/Signal value detected �?????T HOLD")
        ...
        if (
            ...
            logger.info("MACD crossover up �?????T BUY")
            sig = "BUY"
        elif (
            macd.iloc[-2] > signal_line.iloc[-2]
            and macd.iloc[-1] < signal_line.iloc[-1]
        ):
            logger.info("MACD crossover down �?????T SELL")
            sig = "SELL"
        elif macd.iloc[-1] > signal_line.iloc[-1]:
            logger.info("MACD above signal �?????T BUY (trend confirmation)")
            sig = "BUY"
        elif macd.iloc[-1] < signal_line.iloc[-1]:
            logger.info("MACD below signal �?????T SELL (trend confirmation)")
            sig = "SELL"
        else:
            logger.info("MACD holds inside range �?????T HOLD")
            sig = "HOLD"
