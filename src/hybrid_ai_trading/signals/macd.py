"""
MACDSignal (Hybrid AI Quant Pro v2.3 â€“ Hedge-Fund Grade, Test-Aligned)
----------------------------------------------------------------------
Responsibilities:
- Compute MACD (EMA12 â€“ EMA26) and Signal line (EMA9 of MACD)
- Detect crossovers:
...
class MACDSignal:
    ...
    def generate(
        ...
        if closes.empty:
            logger.error("Invalid: no 'c' fields found in bars â†’ HOLD")
            return {"signal": "HOLD", "reason": "invalid"}

        if len(closes) < self.slow + self.signal_window:
            logger.info("Not enough bars (%s) â†’ HOLD", len(closes))
        ...
        if math.isnan(macd.iloc[-1]) or math.isnan(signal_line.iloc[-1]):
            logger.warning("NaN MACD/Signal value detected â†’ HOLD")
        ...
        if (
            ...
            logger.info("MACD crossover up â†’ BUY")
            sig = "BUY"
        elif (
            macd.iloc[-2] > signal_line.iloc[-2]
            and macd.iloc[-1] < signal_line.iloc[-1]
        ):
            logger.info("MACD crossover down â†’ SELL")
            sig = "SELL"
        elif macd.iloc[-1] > signal_line.iloc[-1]:
            logger.info("MACD above signal â†’ BUY (trend confirmation)")
            sig = "BUY"
        elif macd.iloc[-1] < signal_line.iloc[-1]:
            logger.info("MACD below signal â†’ SELL (trend confirmation)")
            sig = "SELL"
        else:
            logger.info("MACD holds inside range â†’ HOLD")
            sig = "HOLD"
