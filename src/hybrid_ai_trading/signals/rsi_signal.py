"""
RSISignal (Hybrid AI Quant Pro v23.3 â€“ Hedge-Fund Grade, Wrapper-Aligned)
-------------------------------------------------------------------------
Logic:
- Compute RSI using Wilderâ€™s method
- BUY if RSI < 30
- SELL if RSI > 70
- HOLD otherwise
- Guards:
...
class RSISignal:
    ...
    def generate(
        ...
        try:
            ...
            logger.error("âŒ Failed to parse closes for RSI: %s", e)
        ...
        try:
            ...
            logger.error("âŒ RSI calculation failed: %s", e)
