...
def main() -> None:
    ...
    if risk["risk_off"]:
        print_line(
            f"BLACK SWAN: BTC 24h drop ?f��?,?�?<??? {risk['metric']:.1f}% ?f��?,?�?,� {risk['threshold']:.1f}% -> NO NEW BUYS"
        )
        lev = 0.0
    else:
    ...
    for pair in crypto_pairs:
        ...
        if sig["buy"]:
            ...
            print_line(
                f"{pair} ({sig['tf']}): BUY  entry?f��?,?�?<???{sig['last']:.2f}  size?f��?,?�?<???{eff_size:.2f}  "
                f"stop?f��?,?�?<???{stop_txt}  (ATR={sig['atr']:.2f})  {sig['reason']}"
            )
    ...
    if not any_buy:
        ...
        if near:
            ...
            for it in near[:5]:
                print_line(
                    f"  {it['pair']}: last?f��?,?�?<???{it['last']:.2f} stop?f��?,?�?<???SMA50 {it['sma50']:.2f}"
                )
    if suppressed:
    ...
    if kr_hold:
        for pair in crypto_pairs:
            ...
            if sig["sell"]:
                ...
                stop_txt = f"{sig['stop']:.2f}" if sig["stop"] is not None else "n/a"
                print_line(
                    f"{pair}: SELL  qty?f��?,?�?<???{amt:.8f}  last?f��?,?�?<???{sig['last']:.2f}  stop?f��?,?�?<???{stop_txt}  ({sig['reason']})"
                )
    else:
            ...
            if _active_window():
                ...
                if cur != last:
                    ...
                    if cur["buys"]:
                        for b in cur["buys"]:
                            print_line(
                                f"BUY {b['p']} size?f��?,?�?<???{b['size']} stop?f��?,?�?<???{b['stop']}"
                            )
                    else:
                        print_line("(no buys)")
                    if cur["sells"]:
